 ...
 */
public class ClientChecker implements ClientPlugin{
    ...
    /**
     * Проверяет на валидность РНН физ. лица
     * @param str РНН
     * @return true - валидна, false - иначе
     */
    ...
    public boolean checkIN(String str) {
        ...
        private int getControlSum(String code) {
        ...
        private int calculate_Checking_numeral(String code) {
                int a[] = getNumeric(code);
                int number = a[0] + 2 * a[1] + 3 * a[2] + 4 * a[3] + 5 * a[4] + 6 * a[5] +
                        7 * a[6] + 8 * a[7] + 9 * a[8] + 10 * a[9] + 11 * a[10];
                number %= 11;
