...
import com.cifs.or2.client.gui.OrEnum;
...
public class Utils {
    ...
    public static Pair[] parsePath(String path) throws KrnException {
        return (InterfaceManagerFactory.instance().getManager() != null)
            ? parsePath(path, InterfaceManagerFactory.instance().getManager().getKernel())
            : parsePath(path, Kernel.instance());
    }
    ...
    public static String getPathForMethod(KrnMethod method){    	
    	return method.name;
    }
    
    public static String getPathForClass(KrnClass cls){
    	return (cls != null) ? cls.name : "";
    }
    
    public static String getPathForClasses(KrnClass[] classes){
    	String res = "";
    	if(classes != null && classes.length > 0){
    		StringBuffer sb;
    		if(classes.length == 1){
    			sb = new StringBuffer(classes[0].name);
    		}else{
    			sb = new StringBuffer(classes[1].name);
    ...
    public static long getDataLangId() {
        return (InterfaceManagerFactory.instance().getManager() != null)
            ? getDataLangId(InterfaceManagerFactory.instance().getManager().getKernel())
            : getDataLangId(Kernel.instance());
    }
