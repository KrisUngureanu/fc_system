...
import static kz.tamur.rt.data.RecordStatus.CREATED;
import static kz.tamur.rt.data.RecordStatus.INSERTED;
import static kz.tamur.rt.data.RecordStatus.MODIFIED;
import static kz.tamur.rt.data.RecordStatus.NaD;
import static kz.tamur.rt.data.RecordStatus.UPTODATE;
import static kz.tamur.util.CacheChangeRecord.CHANGE_ATTR;
import static kz.tamur.util.CacheChangeRecord.CREATE_OBJECT;
import static kz.tamur.util.CacheChangeRecord.DELETE_ATTR;
import static kz.tamur.util.CacheChangeRecord.DELETE_OBJECT;
...
import com.cifs.or2.kernel.DateChange;
import com.cifs.or2.kernel.DateValue;
import com.cifs.or2.kernel.DelRecord;
import com.cifs.or2.kernel.FloatChange;
import com.cifs.or2.kernel.FloatValue;
import com.cifs.or2.kernel.KrnAttribute;
import com.cifs.or2.kernel.KrnClass;
import com.cifs.or2.kernel.KrnException;
import com.cifs.or2.kernel.KrnObject;
import com.cifs.or2.kernel.LongChange;
...
import com.cifs.or2.kernel.StringChange;
import com.cifs.or2.kernel.StringValue;
import com.cifs.or2.kernel.TimeChange;
...
public class Cache {
	...
	private Map<Long, List<CacheAction>> actions = new HashMap<Long, List<CacheAction>>();
	...
	private Map<Long, Set<KrnAttribute>> lattrs = new HashMap<Long, Set<KrnAttribute>>();
...
/*						
						if (attr.rAttrId > 0) {
							if (isLinkAttr(attr.rAttrId)) {
								for (long objId : lostIds) {
									List<CacheAction> acts = actions.get(objId);
									for (CacheAction act : acts) {
										if (act.action == ACT_ADD) {
											addToRevAttrObject(act.obj, act.lattrs, act.val);
										} else if (act.action == ACT_REMOVE) {
											removeFromRevAttrObject(act.obj, act.lattrs, act.val);
										}
									}
									actions.remove(objId);
									
									AttrRecord head = new AttrRecord(objId, attr.id, langId, 0);
									AttrRecord tail = new AttrRecord(objId, attr.id, langId, Integer.MAX_VALUE);
									SortedSet<AttrRecord> set = recs.subSet(head, tail);
									if (set.size() > 0) {
										for (Record r : set) {
											int st = r.getStatus();
	...
	public Record getRecord(final long objId, final KrnAttribute attr, final long langId, final Object value) throws KrnException {////2
		SortedSet<Record> set = getRecords(new long[]{objId}, attr, langId, null);
		Record res = Funcs.findByValue(set, objId, attr.id, langId, value);
		return (res != null) ? (res.getValue() != null) ? res : null : null;
	}

	public SortedSet<ObjectRecord> getObjects(final long classId,
			final long filterId, final RecordsReceiver receiver)
			throws KrnException {
		return getObjects(classId, filterId, new int[1], receiver);
	}
	
	public SortedSet<ObjectRecord> getObjects(final long classId,
			final long filterId, int[] limit, final RecordsReceiver receiver)
			throws KrnException {

		ObjectRecord low = new ObjectRecord(classId);
		ObjectRecord high = new ObjectRecord(classId + 1);
		SortedSet<ObjectRecord> res = objRecs.subSet(low, high);
		if (res.isEmpty() || filterId != 0) {
			...
			for (int i = 0; i < os.length; ++i) {
				objRecs.add(new ObjectRecord(classId, os[i]));
				putObject(os[i]);
			}
...
// TODO links
			/*
			Set<KrnAttribute> lattrs = getLinkAttrs(rec.getAttrId());

			if (lattrs != null && lattrs.size() > 0) {
				if (oldValue instanceof KrnObject) {

					addCacheAction((KrnObject) oldValue, (KrnObject) val, lattrs, ACT_REMOVE);
				}
				if (originator != this && val != null) {
					// Добавляем объект в обратные атрибуты нового значения
					addCacheAction(obj, (KrnObject) val, lattrs, ACT_ADD);
				}
			}*/
		}
			...
			if (contains(objSuperClasses, cls)) {//если тип класса обратного атрибута,
				...
				if (rattr.collectionType == COLLECTION_SET
					...
					if (r == null) {
						...
						attrChanged = changeObjectAttribute(r, obj, this);
					}
					// Удаляем запись из лога, т.к. происходит изменение обратного
					// атрибута
					if (attrChanged) changes.remove(changes.size() - 1);
				}
			}
		}
	}
    ...
    private void addCacheAction(KrnObject obj, KrnObject val, Set<KrnAttribute> lattrs, int actRemove) {
		CacheAction act = new CacheAction(obj, val, lattrs, ACT_REMOVE);
		
		List<CacheAction> l = actions.get(val.id);
		if (l == null) {
			l = new ArrayList<CacheAction>();
			actions.put(val.id, l);
		}
		l.add(act);
	}
	...
	public void deleteObjectAttribute(Record rec, Object originator, boolean isSetToNull)
		...
		if (rec != null) {
			...
			// Обработка линков
			Set<KrnAttribute> rattrs = getRevAttrs(rec.getAttrId());
			// TODO links if (rattrs == null || rattrs.size() == 0) rattrs = getLinkAttrs(rec.getAttrId());
...
// TODO links
		/*
		Set<KrnAttribute> lattrs = getLinkAttrs(attr.id);
    ...
    public void moveRowsBeforeObjectAttribute(KrnObject obj, KrnAttribute attr,
            int i, int[] rows, long langId, Object originator)
            throws KrnException {
        ...
        if (set != null) {
          Map<AttrRecord, Object> indexes = new HashMap<AttrRecord,  Object>();
...
//                          attrRecord.setIndex(i - beforeI + k);

                          AttrRecord head1 = new AttrRecord(obj.id, attr.id, langId, i - beforeI + k);
                          AttrRecord tail1 = new AttrRecord(obj.id, attr.id, langId, i - beforeI + k + 1);
...
//                          attrRecord.setIndex(i - beforeI + k);
              ...
              if (!done) {
                    //attrRecord.setIndex(index + shift);
                  AttrRecord head1 = new AttrRecord(obj.id, attr.id, langId, index + shift);
                  AttrRecord tail1 = new AttrRecord(obj.id, attr.id, langId, index + shift + 1);
                  SortedSet<AttrRecord> set1 = recs.subSet(head1, tail1);
                  AttrRecord attr1 = set1.first();
                  indexes.put(attr1, obj1);
              }
...
*/
  ...
  public void changePlacesObjectAttribute(KrnObject obj, KrnAttribute attr,
	...
	private SortedSet<AttrRecord> addToCash(Set<Long> objectIds,
			KrnAttribute attr, long langId, long[] filterIds, AttrRequest req) throws KrnException {
		...
		if (attr.typeClassId == Kernel.IC_INTEGER
			...
			for (long id : ids) {
				...
				for (int i = 0; i < vs.length; i++) {
					...
					if (v.length > 0) {
						try {
							f = Funcs.createTempFile("blob", null);
							f.deleteOnExit();
			...
			if (req != null && req.getChildren().size() > 0) {
				QueryResult qr = krn.getObjectValues(ids, attr.classId, attr, filterIds, tid, req);
				...
				ObjectValue[] vs = krn.getObjectValues(ids, attr.classId, attr, filterIds, tid);
	...
	private boolean isLinkAttr(long attrId) throws KrnException {
		synchronized (rattrs) {
			Set<KrnAttribute> attrs = rattrs.get(attrId);
			if (attrs == null || attrs.size() == 0) {
				synchronized (lattrs) {
					Set<KrnAttribute> attrs2 = lattrs.get(attrId);
	...
	private void moveLinkAttr(long attrId) throws KrnException {
		synchronized (rattrs) {
			Set<KrnAttribute> attrs = rattrs.get(attrId);
			if (attrs == null || attrs.size() == 0) {
				synchronized (lattrs) {
	...
	private Set<KrnAttribute> getLinkAttrs(long attrId) throws KrnException {
		synchronized (lattrs) {
			return lattrs.get(attrId);
		}
	}
	...
	private void putRevAttrs(KrnAttribute attr) throws KrnException {
		synchronized (rattrs) {
			if (attr.rAttrId != 0) {
				...
				/*
				synchronized (lattrs) {
					l = lattrs.get(attr.id);
					if (l == null) {
						l = new HashSet<KrnAttribute>();
						lattrs.put(attr.id, l);
						KrnAttribute[] ras = krn.getLinkAttributes(attr.id);
						for (KrnAttribute ra : ras) {
							l.add(ra);
						}
					}
	...
	public SortedSet<ObjectRecord> getObjects(
			final long classId,
			AttrRequest rootReq,
			final long filterId,
			int[] limit,
		...
		if (res.isEmpty() || filterId != 0) {
			...
			List<Object[]> rows = krn.getClassObjects(cls, rootReq, ufids, limit, tid, info);
			for (Object[] row : rows) {
				KrnObject obj = (KrnObject)row[0];
				objRecs.add(new ObjectRecord(classId, (KrnObject)row[0]));
				putObject(obj);
			}
	...
	private class CacheAction {
		...
		private KrnObject val;
		private Set<KrnAttribute> lattrs;
		private int action;

		public CacheAction(KrnObject obj, KrnObject val,
				Set<KrnAttribute> lattrs, int action) {
			super();
			this.obj = obj;
			this.val = val;
			this.lattrs = lattrs;
