...
function resizeMaps() {
	$('.or3-gis-map').each(function(i) {
		...
		if (gisCoeffsMap[uid] != null) {
			var bounds = gisMapsMap[uid].getView().calculateExtent(gisMapsMap[uid].getSize());
			var coeffs = gisCoeffsMap[uid];
			var bounds2 = [bounds[0]/coeffs[0], bounds[1]/coeffs[1], bounds[2]/coeffs[2], bounds[3]/coeffs[3]];
			
			try {
				gisMapsMap[uid].getView().fit(bounds2, gisMapsMap[uid].getSize());
			} catch (e) {
				console.log(e);
			}
		} else {
			createMap(uid, boundsMap[uid], false);
		}
	});
}
			...
			if ($("#tasksList .portletTasks table tr").length>0) {
				...
				$.each($('#tasksList .portletTasks table tr:first-child td'), function(i,td) {
					if (i==0 || i==1) {
						...
						$(td).width("22%");
						$(".process-header .ph-cell:nth-child("+(i+1)+")").width("22%");
					}else if (i==4 ) {
						$(td).width("12%");
						$(".process-header .ph-cell:nth-child("+(i+1)+")").width("12%");
					}else if (i==5 ) {
						$(td).width("8%");
						$(".process-header .ph-cell:nth-child("+(i+1)+")").width("8%");
					}
				});
			}
		},
...
function initProfile() {
	...
	$.post(window.mainUrl + "&getAttr&rnd=" + rnd(), par, function(res) {
		...
		if (res.result) {
			...
			$("#userDiv").prepend("<div class='ava'><img src='"+"data:image/png;base64," + res.result+"'></div>");
		}
...
$.fn.datetimebox.defaults.onSelect = function(date) {
	...
	if (id == 'cellDatabox') {
		...
		setValue(id, d + '.' + m + '.' + y + ' ' + h + ':' + min + (secs ? ':' + sec : ''));
