 ...
 */
public class OrHTMLImage extends View {
    /**
     ...
     */
    public OrHTMLImage(Element elem) {
        super(elem);
        fBounds = new Rectangle();
        imageObserver = new OrHTMLImage.ImageHandler();
    ...
    private Icon makeIcon(final String gifFile) {
        ...
        try {
        	Funcs.writeStream(in, out, 10000000);
        } catch (IOException e) {
        	System.err.println("ImageView: Couldn't load image icon: " + gifFile);
        } finally {
        	try {
        		in.close();
        		out.flush();
...
// clear out width/height/realoadimage flag and set loading flag
    ...
    private void loadImage() {
        ...
        if (src != null) {
            ...
            } else if (src.equals("jumper")) {
                newImage = kz.tamur.rt.Utils.getImageIcon("jumper_img").getImage();
            } else if (src.equals("attach")) {
                newImage = kz.tamur.rt.Utils.getImageIcon("attachment").getImage();
            } else {
                if (src.length() > 0) {
                        newImage = Toolkit.getDefaultToolkit().createImage(kz.tamur.rt.Utils.decodeImage(src));
                }
...
// for it.
    ...
    private void updateAltTextView() {
        ...
        if (text != null) {
            OrHTMLImage.ImageLabelView newView;

            newView = new OrHTMLImage.ImageLabelView(getElement(), text);
            synchronized (this) {
                altView = newView;
            }
        }
