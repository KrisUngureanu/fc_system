...
import kz.tamur.comps.ui.ext.FocusType;
import kz.tamur.comps.ui.ext.ShadeType;
...
public class LafUtils {
    ...
    public static void drawAlphaBackground(Graphics2D g2d, Rectangle rectangle) {
        drawAlphaBackground(g2d, rectangle.x, rectangle.y, rectangle.width, rectangle.height);
    }

    public static void drawAlphaBackground(Graphics2D g2d, int x, int y, int width, int height) {
        drawAlphaBackground(g2d, x, y, width, height, StyleConstants.ALPHA_RECT_SIZE);
    }

    public static void drawAlphaBackground(Graphics2D g2d, Rectangle rectangle, int size) {
        drawAlphaBackground(g2d, rectangle.x, rectangle.y, rectangle.width, rectangle.height, size);
    }

    public static void drawAlphaBackground(Graphics2D g2d, int x, int y, int width, int height, int size) {
        drawAlphaBackground(g2d, x, y, width, height, size, StyleConstants.LIGHT_ALPHA, StyleConstants.DARK_ALPHA);
    }

    public static void drawAlphaBackground(Graphics2D g2d, Rectangle rectangle, int size, Color light, Color dark) {
        drawAlphaBackground(g2d, rectangle.x, rectangle.y, rectangle.width, rectangle.height, size, light, dark);
    }

    public static void drawAlphaBackground(Graphics2D g2d, int x, int y, int width, int height, int size, Color light, Color dark) {
    ...
    public static Shape drawWebStyle(Graphics2D g2d, JComponent component, Color shadeColor, int shadeWidth, int round,
        ...
        return drawWebStyle(g2d, component, shadeColor, shadeWidth, round, fillBackground, webColored,
                StyleConstants.darkBorderColor, StyleConstants.disabledBorderColor);
    }

    public static Shape drawWebStyle(Graphics2D g2d, JComponent component, Color shadeColor, int shadeWidth, int round,
            boolean fillBackground, boolean webColored, float opacity) {
        return drawWebStyle(g2d, component, shadeColor, shadeWidth, round, fillBackground, webColored,
                StyleConstants.darkBorderColor, StyleConstants.disabledBorderColor, opacity);
    }
    ...
    public static Shape drawWebStyle(Graphics2D g2d, JComponent component, Color shadeColor, int shadeWidth, int round,
        ...
        if (fillBackground) {
            ...
            g2d.setPaint(webColored ? getWebGradientPaint(0, shadeWidth, 0, component.getHeight() - shadeWidth) : component
                    .getBackground());

            if (round > 0) {
                g2d.fillRoundRect(shadeWidth, shadeWidth, component.getWidth() - shadeWidth * 2, component.getHeight()
                        - shadeWidth * 2, round * 2 + 2, round * 2 + 2);
            } else {
                g2d.fillRect(shadeWidth, shadeWidth, component.getWidth() - shadeWidth * 2, component.getHeight() - shadeWidth
                        * 2);
            }
        }
    ...
    private static Map<StringBuilder, GradientPaint> gradientCache = new HashMap<StringBuilder, GradientPaint>();
    ...
    public static GradientPaint getWebGradientPaint(int x1, int y1, int x2, int y2) {
        return getGradientPaint(x1, y1, x2, y2, StyleConstants.topBgColor, StyleConstants.bottomBgColor);
    }

    public static GradientPaint getWebSelectedGradientPaint(Rectangle bounds) {
        return getWebGradientPaint(bounds.x, bounds.y, bounds.x, bounds.y + bounds.height);
    }

    public static GradientPaint getWebSelectedGradientPaint(int x1, int y1, int x2, int y2) {
        return getGradientPaint(x1, y1, x2, y2, StyleConstants.topSelectedBgColor, StyleConstants.bottomSelectedBgColor);
    }

    public static GradientPaint getGradientPaint(int x1, int y1, int x2, int y2, Color color1, Color color2) {
        return getGradientPaint(x1, y1, x2, y2, color1, color2, false);
    }

    public static GradientPaint getGradientPaint(int x1, int y1, int x2, int y2, Color color1, Color color2, boolean cyclic) {
        StringBuilder key = new StringBuilder(100);
        key.append(x1).append(";").append(y1).append(";").append(x2).append(";").append(y2).append(color1).append(color1)
                .append(cyclic);
        if (gradientCache.containsKey(key)) {
            return gradientCache.get(key);
        } else {
            GradientPaint gp = new GradientPaint(x1, y1, color1, x2, y2, color2, cyclic);
    ...
    public static void drawCustomWebBorder(Graphics2D g2d, JComponent component, Shape borderShape, Color shadeColor,
            int shadeWidth, boolean fillBackground, boolean webColored) {
        drawCustomWebBorder(g2d, component, borderShape, shadeColor, shadeWidth, fillBackground, webColored, Color.GRAY,
                Color.LIGHT_GRAY);
    }

    public static void drawCustomWebBorder(Graphics2D g2d, JComponent component, Shape borderShape, Color shadeColor,
            int shadeWidth, boolean fillBackground, boolean webColored, Color border, Color disabledBorder) {
        Object aa = setupAntialias(g2d);
        ...
        if (fillBackground) {
            if (webColored) {
                ...
                g2d.setPaint(new GradientPaint(0, shapeBounds.y, StyleConstants.topBgColor, 0,
                        shapeBounds.y + shapeBounds.height, StyleConstants.bottomBgColor));
    ...
    public static Shape getWebFocusShape(JComponent component, FocusType focusType, int shadeWidth, int round) {
        ...
        // Final focus shape
        if (round > 0) {
            return new RoundRectangle2D.Double(shadeWidth + spacing, shadeWidth + spacing, component.getWidth() - shadeWidth * 2
                    - spacing * 2 - 1, component.getHeight() - shadeWidth * 2 - spacing * 2 - 1, round * 2, round * 2);
        } else {
            return new Rectangle2D.Double(shadeWidth + spacing, shadeWidth + spacing, component.getWidth() - shadeWidth * 2
                    - spacing * 2 - 1, component.getHeight() - shadeWidth * 2 - spacing * 2 - 1);
        }
    }
    ...
    public static void drawShade(Graphics2D g2d, Shape shape, ShadeType shadeType, Color shadeColor, int width, Shape clip,
        ...
        // Applying clip
        Shape oldClip = intersectClip(g2d, clip);
        ...
        if (shadeType.equals(ShadeType.simple)) {
            ...
            if (StyleConstants.simpleShadeTransparency < 1f) {
                g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, StyleConstants.simpleShadeTransparency
                        * currentComposite));
            }
    ...
    public static void drawWebSelection(Graphics2D g2d, Color color, Rectangle selection, boolean resizableLR,
            boolean resizableUD, boolean drawConnectors) {
    ...
    public static void drawWebSelection(Graphics2D g2d, Color color, Rectangle selection, boolean resizableLR,
            boolean resizableUD, boolean drawConnectors, boolean drawSideControls) {
        selection = GeometryUtils.verifyRect(selection);
        ...
        if (resizableUD) {
            if (resizableLR) {
                buttonsShape.add(new Area(new Ellipse2D.Double(selection.x - halfButton, selection.y - halfButton,
                        halfButton * 2, halfButton * 2)));
                buttonsShape.add(new Area(new Ellipse2D.Double(selection.x + selection.width - halfButton, selection.y
                        - halfButton, halfButton * 2, halfButton * 2)));
            }
            if (drawSideControls) {
                buttonsShape.add(new Area(new Ellipse2D.Double(selection.x + selection.width / 2 - halfButton, selection.y
                        - halfButton, halfButton * 2, halfButton * 2)));
            }
        }
        ...
        if (resizableLR && drawSideControls) {
            buttonsShape.add(new Area(new Ellipse2D.Double(selection.x - halfButton, selection.y + selection.height / 2
                    - halfButton, halfButton * 2, halfButton * 2)));
            buttonsShape.add(new Area(new Ellipse2D.Double(selection.x + selection.width - halfButton, selection.y
                    + selection.height / 2 - halfButton, halfButton * 2, halfButton * 2)));
        }

        // Bottom
        if (resizableUD) {
            if (resizableLR) {
                buttonsShape.add(new Area(new Ellipse2D.Double(selection.x - halfButton, selection.y + selection.height
                        - halfButton, halfButton * 2, halfButton * 2)));
                buttonsShape.add(new Area(new Ellipse2D.Double(selection.x + selection.width - halfButton, selection.y
                        + selection.height - halfButton, halfButton * 2, halfButton * 2)));
            }
            if (drawSideControls) {
                buttonsShape.add(new Area(new Ellipse2D.Double(selection.x + selection.width / 2 - halfButton, selection.y
                        + selection.height - halfButton, halfButton * 2, halfButton * 2)));
        ...
        // Button connectors
        if (drawConnectors) {
            Area selectionShape = new Area(new RoundRectangle2D.Double(selection.x - halfLine, selection.y - halfLine,
                    selection.width + halfLine * 2, selection.height + halfLine * 2, 5, 5));
            selectionShape.subtract(new Area(new RoundRectangle2D.Double(selection.x + halfLine, selection.y + halfLine,
    ...
    public static void drawWebSelector(Graphics2D g2d, Color color, Rectangle selection, int selector) {
        ...
        if (selector == SwingConstants.NORTH_WEST) {
            buttonsShape = new Ellipse2D.Double(selection.x - halfSelector, selection.y - halfSelector, halfSelector * 2,
                    halfSelector * 2);
        } else if (selector == SwingConstants.NORTH) {
            buttonsShape = new Ellipse2D.Double(selection.x + selection.width / 2 - halfSelector, selection.y - halfSelector,
                    halfSelector * 2, halfSelector * 2);
        } else if (selector == SwingConstants.NORTH_EAST) {
            buttonsShape = new Ellipse2D.Double(selection.x + selection.width - halfSelector, selection.y - halfSelector,
                    halfSelector * 2, halfSelector * 2);
        } else if (selector == SwingConstants.WEST) {
            buttonsShape = new Ellipse2D.Double(selection.x - halfSelector, selection.y + selection.height / 2 - halfSelector,
                    halfSelector * 2, halfSelector * 2);
        } else if (selector == SwingConstants.EAST) {
            buttonsShape = new Ellipse2D.Double(selection.x + selection.width - halfSelector, selection.y + selection.height / 2
                    - halfSelector, halfSelector * 2, halfSelector * 2);
        } else if (selector == SwingConstants.SOUTH_WEST) {
            buttonsShape = new Ellipse2D.Double(selection.x - halfSelector, selection.y + selection.height - halfSelector,
                    halfSelector * 2, halfSelector * 2);
        } else if (selector == SwingConstants.SOUTH) {
            buttonsShape = new Ellipse2D.Double(selection.x + selection.width / 2 - halfSelector, selection.y + selection.height
                    - halfSelector, halfSelector * 2, halfSelector * 2);
        } else if (selector == SwingConstants.SOUTH_EAST) {
            buttonsShape = new Ellipse2D.Double(selection.x + selection.width - halfSelector, selection.y + selection.height
                    - halfSelector, halfSelector * 2, halfSelector * 2);
        } else {
            buttonsShape = null;
        }
    ...
    public static void drawEtchedShape(Graphics2D g2d, BufferedImage topBg, BufferedImage bottomBg, Shape fullShape,
            Shape bevelShape) {
        Object aa = setupAntialias(g2d);

        Rectangle bounds = fullShape.getBounds();

        g2d.setPaint(new TexturePaint(topBg, new Rectangle(bounds.getLocation(), new Dimension(topBg.getWidth(), topBg
                .getHeight()))));
        g2d.fill(fullShape);
        ...
        g2d.setClip(newClip);
        g2d.setPaint(new TexturePaint(bottomBg, new Rectangle(bounds.getLocation(), new Dimension(bottomBg.getWidth(), bottomBg
                .getHeight()))));
        g2d.fill(bevelShape);
     ...
     * possible. The shadow is generated with multiple calls to draw string. This method paints the text on coordinates 0, 1. If text should
