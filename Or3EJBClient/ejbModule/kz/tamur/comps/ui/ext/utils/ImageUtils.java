...
import kz.tamur.comps.ui.ext.graphics.filters.ImageFilterUtils;
import kz.tamur.comps.ui.ext.graphics.filters.ShadowFilter;
...
public class ImageUtils {
    ...
    public static BufferedImage toCompatibleImage(BufferedImage image) {
        ...
        BufferedImage compatibleImage = SystemUtils.getGraphicsConfiguration().createCompatibleImage(image.getWidth(),
    ...
    public static BufferedImage createColorImage(Color color, int width, int height) {
        ...
        g2d.drawRoundRect(0, 0, width - 1, height - 1, StyleConstants.largeRound, StyleConstants.round);
        g2d.setPaint(Color.WHITE);
        g2d.drawRoundRect(1, 1, width - 3, height - 3, StyleConstants.round, StyleConstants.round);
        g2d.setPaint(color);
        g2d.fillRoundRect(2, 2, width - 4, height - 4, StyleConstants.round, StyleConstants.round);
        g2d.dispose();
        return image;
    }
    ...
    public static boolean isImageLoadable(String name) {
        return Constants.IMAGE_FORMATS.contains(FileUtils.getFileExtPart(name, false).toLowerCase(Locale.ROOT));
    }
        ...
        synchronized (thumbnailsCache) {
            if (thumbnailsCache.size() > 0) {
                ...
                for (String key : thumbnailsCache.keySet()) {
                    if (key.endsWith(IMAGE_CACHE_SEPARATOR + src)) {
                        toRemove.add(key);
                    }
                }
    ...
    public static ImageIcon getImageThumbnailIcon(String prefix, String src, int size) {
        synchronized (thumbnailsCache) {
            if (src != null && !src.trim().equals("")) {
                if (thumbnailsCache.containsKey(prefix + size + IMAGE_CACHE_SEPARATOR + src)) {
                    ImageIcon imageIcon = thumbnailsCache.get(prefix + size + IMAGE_CACHE_SEPARATOR + src);
                    if (imageIcon != null) {
                        return imageIcon;
                    } else {
                        thumbnailsCache.remove(prefix + size + IMAGE_CACHE_SEPARATOR + src);
                    ...
                    if (icon != null) {
                        ...
                        thumbnailsCache.put(prefix + size + IMAGE_CACHE_SEPARATOR + src, imageIcon);

                        if (imageIcon != null) {
                            // Saving image size
                            imageIcon.setDescription(icon.getIconWidth() + "x" + icon.getIconHeight());

                            // Removing forcely loaded images from memory
                            if (!wasCached) {
                                clearImageCache(src);
                            }
                        }
                        ...
                        // Caching empty image
                        thumbnailsCache.put(prefix + size + IMAGE_CACHE_SEPARATOR + src, icon);
                        return icon;
                    }
                }
            } else {
    ...
    public static BufferedImage createPreviewImage(BufferedImage image, int length) {
        ...
        if (width >= 3 && height >= 3) {
            ...
            byte[] img = null;
            try {
                img = getScaledImage(imageBytes, width, height, "PNG");
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public static BufferedImage getBufferedImage(Image image) {
        if (image == null || image.getWidth(null) <= 0 || image.getHeight(null) <= 0) {
            ...
            BufferedImage bufferedImage = createCompatibleImage(image.getWidth(null), image.getHeight(null),
    ...
    public static Image getBufferedImage(Icon icon) {
        if (icon == null) {
            ...
            BufferedImage bufferedImage = createCompatibleImage(icon.getIconWidth(), icon.getIconHeight(),
                    Transparency.TRANSLUCENT);
            Graphics2D g2d = bufferedImage.createGraphics();
    ...
    public static ImageIcon getDisabledCopy(String id, ImageIcon imageIcon) {
        if (grayscaleCache.containsKey(id)) {
            return grayscaleCache.get(id);
        } else {
            grayscaleCache.put(id, createDisabledCopy(imageIcon));
            return grayscaleCache.get(id);
        }
    }
    ...
    public static BufferedImage createPrettyImage(BufferedImage bufferedImage, int shadeWidth, int round) {
        ...
        BufferedImage bi = createCompatibleImage(width + shadeWidth * 2 + 1, height + shadeWidth * 2 + 1,
                Transparency.TRANSLUCENT);
        Graphics2D g2d = bi.createGraphics();
        LafUtils.setupAntialias(g2d);
        LafUtils.setupImageQuality(g2d);
        ...
        g2d.setPaint(new LinearGradientPaint(0, shadeWidth, 0, height - shadeWidth, new float[] { 0f, 0.5f, 1f }, new Color[] {
                new Color(125, 125, 125, 48), new Color(125, 125, 125, 0), new Color(125, 125, 125, 48) }));
        g2d.fill(border);
     ...
     *             the exception
     */
    public static byte[] getScaledImage(byte[] imageData, int width, int height, String format) throws Exception {
        BufferedImage img = ImageIO.read(new ByteArrayInputStream(imageData));
        if (img != null) {
            float s = Math.max((float) width / img.getWidth(), (float) height / img.getHeight());
            ...
            res.createGraphics().drawImage(img.getScaledInstance(nw, nh, Image.SCALE_SMOOTH), width > 0 ? (width - nw) / 2 : 0,
                    height > 0 ? (height - nh) / 2 : 0, null);
