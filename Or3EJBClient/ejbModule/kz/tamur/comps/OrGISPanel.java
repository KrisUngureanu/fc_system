 ...
 * The Class OrPanel.
 */
public class OrGISPanel extends JPanel implements OrGuiContainer {
    ...
    /** The hspacer cts. */
    private GridBagConstraints hspacerCts = new GridBagConstraints(0, 0, 1, 1, 1, 0, GridBagConstraints.CENTER, 1, new Insets(0, 0, 0, 0), 0, 0);

    /** The vspacer cts. */
    private GridBagConstraints vspacerCts = new GridBagConstraints(0, 0, 1, 1, 0, 1, GridBagConstraints.CENTER, 1, new Insets(0, 0, 0, 0), 0, 0);
   ...
   /**
     ...
     *             the krn exception
     */
    OrGISPanel(Element xml, int mode, Factory fm, OrFrame frame) throws KrnException {
        this.xml = xml;
        this.mode = mode;
        this.frame = frame;
    ...
    private void init(Factory fm, int mode) throws KrnException {
        ...
        if (mode == Mode.DESIGN) {
            ...
            adapter = new GISPanelAdapter((UIFrame) frame, this, false);
        ...
        if (!pv.isNull()) {
            ...
            for (int i = 0; i < childrenCopy.size(); i++) {
                ...
                if (mode == Mode.DESIGN) {
                    ...
                    } else if (c instanceof OrGISPanel) {
                        OrButton button = ((OrGISPanel) c).getDefaultButton();
                        if (button != null)
                            defaultButton = button;
                    }
                }
...
//		}
    ...
    private void removeEnterKs() {
        ...
        for (Component comp : getComponents()) {
            ...
            if (comp instanceof OrGISPanel)
                ((OrGISPanel) comp).removeEnterKs();
        }
    }
