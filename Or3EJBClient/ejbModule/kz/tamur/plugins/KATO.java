 ...
 */
public class KATO implements ClientPlugin {
	
    final Kernel kernel = Kernel.instance();
    ...
    public KrnObject[] getRegion(KrnObject value, KrnObject lang) {
        try {
            ...
            while ((parent = getParent(obj)) != null) {
                String[] str_value = kernel.getStrings(obj, "мультинаименование", langId, 0);
    ...
    private KrnObject getParent(KrnObject obj) {
        ...
        try {
            KrnObject[] par_obj = kernel.getObjects(obj, "структ классификатора КАТО", 0);
            KrnObject[] parents = kernel.getObjects(par_obj[0], "родитель", 0);
            if (parents.length > 0) {
                KrnObject[] values = kernel.getObjects(parents[0], "значение", 0);
                return values[0];
            }
        } catch (KrnException e) {
            e.printStackTrace();
    ...
    private KrnObject getChild(KrnObject obj) {
        ...
        try {
            KrnObject[] par_obj = kernel.getObjects(obj, "структ классификатора КАТО", 0);
            KrnObject[] children = kernel.getObjects(par_obj[0], "дети", 0);
            if (children.length > 0) {
            	long[] ids = Funcs.makeObjectIdArray(children);
            	ObjectValue[] ovs = kernel.getObjectValues(ids, children[0].classId, "значение", 0);
            	long[] vids = Funcs.makeObjectValueIdArray(ovs);
            	
            	LongValue[] lvs = kernel.getLongValues(vids, ovs[0].value.classId, "лог -удален?-", 0);
