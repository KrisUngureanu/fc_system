...
/*    public static void main(String args[]) {
        try {
            FileInputStream fis = new FileInputStream("xml1.xml");
            SAXBuilder builder = new SAXBuilder();
            Element doc = builder.build(fis).getRootElement();
            List personTags = doc.getChild("Persons").getChildren("Person");

            Element personTag = (Element) personTags.get(0);
            new FLService().checkPersons(personTag);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
