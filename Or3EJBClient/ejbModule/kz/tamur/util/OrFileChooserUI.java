 ...
 * @author Jeff Dinkins
 */
public class OrFileChooserUI extends BasicFileChooserUI {
    ...
    protected ResourceBundle res = ResourceBundle.getBundle(
            Constants.NAME_RESOURCES, Utils.getLocale());
    ...
    public static ComponentUI createUI(JComponent c) {
        return new OrFileChooserUI((JFileChooser) c);
    }

    public OrFileChooserUI(JFileChooser filechooser) {
	super(filechooser);
    }
	...
	// ComboBox Label
     	lookInLabel = Utils.createLabel(lookInLabelText);
     	lookInLabel.setDisplayedMnemonic(lookInLabelMnemonic);
	topPanel.add(lookInLabel, BorderLayout.BEFORE_LINE_BEGINS);

	// CurrentDir ComboBox
	directoryComboBox = new JComboBox();
        directoryComboBox.setFont(Utils.getDefaultFont());
        directoryComboBox.setForeground(Utils.getDarkShadowSysColor());
        directoryComboBox.setBackground(Utils.getLightSysColor());
	...
	upFolderButton.setText(null);
        upFolderButton.setBorder(BorderFactory.createEmptyBorder());
	upFolderButton.setIcon(upFolderIcon);
     	upFolderButton.setToolTipText(res.getString("upFolder"));//"На один уроверь вверх");
	...
	String toolTipText = res.getString("desktop");//homeFolderToolTipText;
	...
	newFolderButton.setIcon(newFolderIcon);
        newFolderButton.setBorder(BorderFactory.createEmptyBorder());
     	newFolderButton.setToolTipText(res.getString("createFolder"));//"Создать папку");
	...
	newFolderButton.setMargin(shrinkwrap);
	...
	listViewButton = new JToggleButton(listViewIcon);
     	listViewButton.setToolTipText(res.getString("list"));//"Список");
        listViewButton.setBorder(BorderFactory.createEmptyBorder());
	...
	detailsViewButton = new JToggleButton(detailsViewIcon);
     	detailsViewButton.setToolTipText(res.getString("table"));//"Таблица");
        detailsViewButton.setBorder(BorderFactory.createEmptyBorder());
	...
	fileNameTextField = new JTextField() {
        ...
        fileNameTextField.setBorder(BorderFactory.createLineBorder(Utils.getDarkShadowSysColor()));
	...
	filterComboBox = new JComboBox(filterComboBoxModel);
        filterComboBox.setFont(Utils.getDefaultFont());
        filterComboBox.setForeground(Utils.getDarkShadowSysColor());
        filterComboBox.setBackground(Utils.getLightSysColor());
	...
	approveButton = ButtonsFactory.createDialogButton(ButtonsFactory.BUTTON_OK);
	// Note: Metal does not use mnemonics for approve and cancel
	approveButton.addActionListener(getApproveSelectionAction());
	approveButton.setToolTipText(getApproveButtonToolTipText(fc));
	getButtonPanel().add(approveButton);

	cancelButton = ButtonsFactory.createDialogButton(ButtonsFactory.BUTTON_CANCEL);
        cancelButton.setText(res.getString("cancel"));
	cancelButton.setToolTipText(res.getString("cancel"));//cancelButtonToolTipText);
	...
	lookInLabelText = res.getString("findIn");//"Поиск в:";
	saveInLabelText = res.getString("saveIn");//"Сохранить в:";
	
	fileNameLabelMnemonic = UIManager.getInt("FileChooser.fileNameLabelMnemonic");  
	fileNameLabelText = res.getString("fileName");//"Имя файла";//UIManager.getString("FileChooser.fileNameLabelText",l);
	
	filesOfTypeLabelMnemonic = UIManager.getInt("FileChooser.filesOfTypeLabelMnemonic");  
	filesOfTypeLabelText = res.getString("fileTypes");//"Типы файлов";//UIManager.getString("FileChooser.filesOfTypeLabelText",l);
	...
	fileNameHeaderText = res.getString("fileName");//"Имя файла";
	fileSizeHeaderText = res.getString("fileSize");//"Размер";
	fileTypeHeaderText = res.getString("fileType");//"Тип";
	fileDateHeaderText = res.getString("fileDate");//"Дата";
	fileAttrHeaderText = res.getString("fileAttr");//"Атрибут";
    }
	...
	public void setValueAt(Object value, int row, int col) {
		...
		if (f != null) {
			...
			if (!f2.exists() && OrFileChooserUI.this.getModel().renameFile(f, f2)) {
	...
	detailsTable.putClientProperty("JTable.autoStartsEdit", Boolean.FALSE);
        detailsTable.setFont(Utils.getDefaultFont());
        detailsTable.setForeground(Utils.getDarkShadowSysColor());
	...
	if (editFile != null && editFile.exists()) {
	    ...
	    String newDisplayName = Funcs.normalizeInput(editCell.getText()).trim();
	    String newFileName;
	...
	public Component getListCellRendererComponent(JList list, Object value,
	    ...
	    Icon icon = null;
            if (file.getName().length() > 3) {
                if ("xml".equals(file.getName().substring(file.getName().length() - 3))) {
                    icon = kz.tamur.rt.Utils.getImageIcon("XML");
                } else if ("gif".equals(file.getName().substring(file.getName().length() - 3))
                    || "jpg".equals(file.getName().substring(file.getName().length() - 3))) {
                    icon = kz.tamur.rt.Utils.getImageIcon("Image");
                } else {
