		...
		success : function(data) {
			if (data.helps && data.helps.length > 0) {
				...
				for (var i = 0; i < data.helps.length; i++) {
					var help = data.helps[i];
					var row = "";
					if (help.type == 2) {
						row = "<div class='help-file-item'>";
						row += "<a href=\"#\" onclick=\"return downloadUrl('&cmd=getHelpNoteContent&hid=" + help.id + "');\" class=\"help-link\">" + help.title + "</a>";
						row += '</div>';
					} else {
						row = "<div class='help-item'>";
						row += "<a target='_blank' href=\"" + window.contextName + '/jsp/help.jsp?guid=' + guid + "&hid=" + help.id + "&title=" + help.title + "\" class='help-link'>" + help.title + "</a>";
						row += '</div>';
					}
					table += row;
				}
			  	var content = "<div class=\"help-name\"><h3>" + translation['helpDocs'] + "</h3>" + table+"</div>";
			  	$('.tool-help').append($(content));
			}
		},
		dataType : 'json',
...
function resizeMaps() {
	//initVisibleMaps();
	if (this.resizeMapsTimer) clearTimeout(this.resizeMapsTimer);
    this.resizeMapsTimer = setTimeout(function() {
    	$('.or3-gis-map').each(function(i) {
    		var uid = $(this).attr('id');
    		
    		if (window['map_' + uid] != null) {
    			window['map_' + uid].updateSize();
    		}
    	});
    }, 700);
}
...
//конструктор
function LostRequest(type, url, params, success, dataType, async) {
	this.type = type;
	this.url = url;
	this.params = params;
	...
	if (noConnection) {
		...
		timeoutId = setTimeout(function() {
			...
			for (var i=0; i<requests.length; i++) {
				var req = requests[i];
				saveAjaxRequest(req.type, req.url, req.params, req.success, req.dataType, req.async);
			}
			requests = null;
			timeoutId = null;
		...
		for (var i=0; i<data.commands.length; i++) {
			...
			if (cmd) {
				if (cmd.refresh != null) {
					...
					const flowId = cmd.start_ui;
					if (or34j_url) {
						let params = { flowId: flowId };
						fetch(`${or34j_url}/form?params=${encodeURIComponent(JSON.stringify(params))}`, { credentials: 'include' })
						.then(response => response.json())
						.then(json => {
							if ('formId' in json) {
				   				window.location.href = `index.egkn.html?guid=${guid}&uid=${sessionUID}&serverId=${serverId}&userId=${userId}&flowId=${flowId}`;
							} else {
								openUi(flowId, activeMenu);
							}
						});
					} else {
						openUi(flowId, activeMenu);
					...
					selectUcgoCertificate(cmd.selectUcgoCertificate);
					...
					window.multiSelection[compUid] = multi;
				} else if (cmd.showDialog) {
					showDialog(cmd.showDialog.title, cmd.showDialog.w, cmd.showDialog.h, cmd.showDialog.clear);
				}
			}
		}
	} 
...
function openUi(flowId, activeMenu) {
	if (document.location.hash.indexOf("cmd=openTask&uid=" + flowId) == -1) {
		document.location.hash = "cmd=openTask&uid=" + flowId + "&id=" + activeMenu + "&rnd=" + rnd();
		if (dialogOpened) {
...
$(function() {
	...
	$(window).on('hashchange', function() {
		var day = escapeHash(location.hash);
		if (prevHash != day) {
			prevHash = day;
			if (day=="")
				return false;
	...
	if ($('#startDiv').length > 0) {
		$('#startDiv').load(window.contextName + '/jsp/start.jsp?guid=' + guid, function() {
			$('.portlet').css('height', ($(window).height()-95)+"px");
	...
	if ($('#helpWnd').length > 0) {
		$('#helpWnd').load(window.contextName + '/jsp/faq.jsp?guid=' + guid, function() {
			...
			saveAjaxRequest('POST', window.mainUrl, par, parseHelpTabs, 'json', true);
		});
	}
...
var taskOrderBy = "date";
var taskOrderDesc = 1;

function loadTasks(rowFirst, rowLast, orderBy, desc) {
	loadingCount++;
	...
	par["rowLast"] = rowLast;
	par["sort"] = taskOrderBy;
	par["desc"] = taskOrderDesc;
	...
	$.ajax({
		...
		success : function(data) {
			...
			//addOr34jHandlers($("a[href^='#cmd=openTask']"));
			setTasksCount(tasksCount);
			loadingCount--;
			if ($("#tasksList .portletTasks table tr").length>0) {
				var tasksTable = $('#tasksList .portletTasks').parent();
				var header = tasksTable.find('.process-header');

				var head = "<div class='process-header'>" +
						"<div class='ph-cell'></div>" +
						"<div class='ph-cell' sort='error'" + (taskOrderBy == 'error' ? " desc='" + taskOrderDesc + "'" : "") + ">...</div>" +
						"<div class='ph-cell' sort='processName'" + (taskOrderBy == 'processName' ? " desc='" + taskOrderDesc + "'" : "") + ">Процесс/Объект обработки<span class='"+ (taskOrderBy == 'processName' ? "order_dir" + taskOrderDesc + "'" : "") +"'></span></div>" +
						"<div class='ph-cell' sort='taskName'" + (taskOrderBy == 'taskName' ? " desc='" + taskOrderDesc + "'" : "") + ">Этап<span class='"+ (taskOrderBy == 'taskName' ? "order_dir" + taskOrderDesc + "'" : "") +"'></span></div>" +
						"<div class='ph-cell' sort='date'" + (taskOrderBy == 'date' ? " desc='" + taskOrderDesc + "'" : "") + ">Дата и время<span class='"+ (taskOrderBy == 'date' ? "order_dir" + taskOrderDesc + "'" : "") +"'></span></div>" +
						"<div class='ph-cell' sort='cdate'" + (taskOrderBy == 'cdate' ? " desc='" + taskOrderDesc + "'" : "") + ">Срок исполн.<span class='"+ (taskOrderBy == 'cdate' ? "order_dir" + taskOrderDesc + "'" : "") +"'></span></div>" +
						"</div>";
				
				if (header.length == 0)
					tasksTable.prepend(head);
				else
					header.replaceWith(head);

				$.each($('#tasksList .portletTasks table tr:first-child td'), function(i,td) {
					if (i==0 || i==1) {
						$(td).width("1%");
						$(".process-header .ph-cell:nth-child("+(i+1)+")").width("1%");
					}else if (i==2 ) {
						$(td).width("56%");
						$(".process-header .ph-cell:nth-child("+(i+1)+")").width("56%");
					}else if (i==3 ) {
						$(td).width("22%");
						$(".process-header .ph-cell:nth-child("+(i+1)+")").width("20%");
					}else if (i==4 ) {
						$(td).width("12%");
						$(".process-header .ph-cell:nth-child("+(i+1)+")").width("12%");
					}else if (i==5 ) {
						$(td).width("8%");
						$(".process-header .ph-cell:nth-child("+(i+1)+")").width("10%");
					}
				});
			}
		},
...
$("#tasksList").on('click', '.ph-cell', function() {
	...
	if (sort != null && sort.length > 0) {
		...
		taskOrderDesc = desc;
		taskOrderBy = sort;
		$(this).find("span").addClass("order_dir"+desc);
...
$("body").on('change', '#processSearchPage', function() {
	indexSearchProces = 1;
		...
		post(par, function(data) {
			...
			if (data.node) {
				...
				if (root.id == nodeId) {
					...
					var html = sanitizeHtml($(root.target).find('.tree-title').text());
			...
			if (data.node) {
				...
				if (root.id == nodeId) {
					...
					$.each(tr.find('td'), function(i, td) {
						...
						var html = sanitizeHtml(tag.text());
						var ind = html.toUpperCase().indexOf(str.toUpperCase());
						if (ind > -1) {
							html = html.substring(0, ind) + '<span class="higliht">' + html.substring(ind, ind + str.length) + '</span>' + html.substring(ind + str.length);
							tag.html(html);
...
function openDocument(id, ext, fn) {
	...
	if (ext == null && fn == null) {
		...
		url += "&sfunc&cls=XmlUtil&name=getHelpFile&ext=" + ext + "&fn=" + encodeURIComponent(fn) + "&arg0=" + id;
	}
	$('#report_frame').attr('src', url);
}

function downloadUrl(part) {
	var url = window.mainUrl + part;
	$('#report_frame').attr('src', url);
	return false;
}
...
$("body").on('click', '.accordion-v .ttl-coll-pan-v', function() {
	console.log ("before: " + $('.or3-gis-map').width() + ", " + $('.or3-gis-map').height());
	...
	console.log ("after: " + $('.or3-gis-map').width() + ", " + $('.or3-gis-map').height());
...
function initProfile() {
	...
	$.post(window.mainUrl + "&getAttr&rnd=" + rnd(), par, function(res) {
		...
		if (res.result) {
			...
			$("#ava").append("<img src='"+"data:image/png;base64," + res.result+"'>");
		}
...
function setSelection(tbl, val, index){
	if(val.length > 0){
		...
		if (window.selectedRow[id] != index) {
			window.clickedGrid = id;
			window.selectedRow[id] = index;
		}
		var selIdx = window.selectedRow[id];
...
$("body").on('click', 'button.or3-note, a.or3-note', function() {
	var uid = $(this).attr("id");
	var title = $(this).find(".btn-label").text();
	
	var url = window.contextName + '/jsp/help.jsp?guid=' + guid + '&rnd=' + rnd() + '&uid=' + uid + '&title=' + title;
...
$("#tasksList").on('click', '.task_check', function(e) {
	var $chkboxes = $('.task_check');
	...
	if(e.shiftKey) {
		...
		$.each($chkboxes.slice(Math.min(start,end), Math.max(start,end)+ 1), function(i,el) {
			
			$(el).prop('checked', lastChecked.checked)
			if (lastChecked.checked)
				$(el).closest('tr').addClass('taskCheckedTr');
			else
				$(el).closest('tr').removeClass('taskCheckedTr');
	...
	if($('.task_check:visible').is(':checked')){
    	$('#taskremoveprocess, #reloadprocess').show();
    	$('#taskremoveprocess').find('.l-btn-text').css('color','#fff');
    } else {
    	$('#taskremoveprocess, #reloadprocess').hide();
    }
...
$("#tasksList").on('click', '#reloadprocess', function() {
	var l = $(".task_check:visible:checked").length;
	if (l > 0) {
		reloadSelectedTasks();
	}
...
$("body").on('change', '.utheme input', function() {
	...
	saveAjaxRequest('POST', window.mainUrl, par, goToProfile, 'html', false);
});
...
function changeInterfaceLang(code) {
	blockPage();
...
function askPassword(doAfterFunc, title) {
	...
	$('#' + dialogId).dialog({
		...
		cache: false,
		closable: true,
		href: window.contextName + '/jsp/enterPassword.jsp?guid=' + guid + '&rnd=' + rnd(),
		modal: true,
		onLoad: function() {
			$('#' + dialogId).parent().find('.panel-title').css({'height' : 'auto'});
		},
...
function generateUcgoPKCS10(dn) {
	...
	if (readers == "") {
		...
		$('#' + dialogId).dialog({
			...
			cache: false,
			closable: true,
			href: window.contextName + '/jsp/selectKeyPairConteiner.jsp?guid=' + guid + '&rnd=' + rnd(),
			modal: true,
			onClose: function() {
				window.dialogResult[dialogId] = '1';
				$("#" + dialogId).dialog('destroy');
...
function selectUcgoCertificateStep2(certs) {
	...
	$('#' + dialogId).dialog({
		...
		cache: false,
		closable: true,
		href: window.contextName + '/jsp/selectCert.jsp?guid=' + guid + '&rnd=' + rnd(),
		modal: true,
		onClose: function() {
			window.dialogResult[dialogId] = '1';
			$("#" + dialogId).dialog('destroy');
...
function signDialog(str, path, pass, cont) {
	...
	if (keyPass == null || keyPath == null || keyPath.length == 0) {
		...
		$('#' + dialogId).dialog({
			...
			cache: false,
			closable: true,
			href: window.contextName + '/jsp/ecp2.jsp?guid=' + guid + '&rnd=' + rnd(),
			modal: true,
			onClose: function() {
				window.dialogResult[dialogId] = '1';
				$("#" + dialogId).dialog('destroy');
...
$("body").on('change', 'input, select, textarea', function() {
	if ($(this).parents('.jb-chat').length > 0) return;
...
// не печатем Enter при редактировании колонки с текстом
$("body").on('keydown', "textarea.datagrid-editable-input", function(e) {
	if (e.which && e.which == 13) {
		console.log("enter ");
		return false;
	}
...
function forseSaveChanges(initiator, callback) {
	breadcrumpsVisible = !($(initiator).length > 0 
						&& $(initiator).attr('id').match(/ui_personInfo|ui_staff|ui_actions|ui_right|ui_stat|ui_kaddelo/));

	endEditing();
	
	var par = {"cmd":"fcommit"};
	
	saveAjaxRequest('POST', window.mainUrl, par, callback, null, true);
}
...
function logout() {
	forseSaveChanges(null, function(res) {
...
function loadUI(hash) {
	...
	if (params.params.ui) {
		...
		hash = hash.replace(/^\#/, '');
		$('#app').attr('init',"1");
		openTask(params.params.uid, window.mainUrl + "&" + hash);
		...
		$('#glassPane').show();
		$('#uiPanel').show();
		if (params.params.id == 'ui_startPage' && params.params.uid) {
			$('#uiTitle').hide();
			$('#uiToolbar').hide();
		} else {
			$('#uiTitle').show();
		...
		$(window).resize();
		$('#app').show();
		$('#app').panel('refresh', window.mainUrl + "&" + hash);
		$('#oldStartDiv,#startDiv, #processesList, #archList, #tasksList, #notiInList, #sessionsList, #archList, #dictsList, #adminsList, #helpWnd, #npa449Wnd, #npa233Wnd').hide();
        clearTimeout(refresher);
	}
	...
	$('.portlet').css('height', ($(window).height()-95)+"px");
	$('.portletTasks').css('height', ($(window).height()-155)+"px");
	...
	if (allProcesses_Layout) {
		...
		if (processesList) {
			$('#processesList').css('height', ($(window).height()-10)+"px");
		}
...
function updateOrders(byType) {
	$.each(byType, function(type, o) {
		$.each(o, function(i, uid) {
			...
			var url = window.mainUrl + "&sfunc&cls=XmlUtil&name=" + method + "&rnd=" + rnd();
			url += "&arg0=" + uid;
			$.get(url, function(data) {
				if (data) {
					checkData(data);
...
function addTask(process) {
	if (tasksCount > -1) {
		...
		$.each(process, function(key, task) {
			...
			//addOr34jHandlers(html.find("a[href^='#cmd=openTask']"));
...
function makeTask(key, task) {
	...
	return '<tr id="' + key + '"><td><input type="checkbox" ' +(task.k == 1 ? 'class="task_check" task="'+key+'"' : 'disabled=""')
		+'/></td><td><span class="' + condition[task.c2] + '">&nbsp;</span></td><td'+(task.c3!=null?' style="background-color: '+task.c3+'"':'')
		+'><h3><a href="' + href + '" class="task-link" task=' + key + '>' + task.p + '</a></h3>'+ task.o+'</td><td class="task-step">' +  task.t + '</td>' 
		+ '<td><span class="dt_ label ' + orderStatus[task.c1] + '"><i class="icon-time icon-white"></i> ' 
		+ task.d + '</span><br><span><i class="icon-user"></i> ' + task.i 
        + '</span></td>'
		+ '<td style="text-align:center">'+ ((task.c.length > 0) ? task.c : translation['mask']) +'</td></tr>';
}
...
function loadSessions() {
	...
	$.get(window.mainUrl + "&cmd=loadSessions", function(data) {
		...
		$.each(processList, function(i, process) {
			$.each(process, function(key, task) {
				var html = '<tr id=' + key + '><td><input type="checkbox" class="session_check" task="' + key + '"></td><td><h3>' + task.p + '</h3>'
					+ '<h4>' + task.t + '</h4>' + task.o 
				...
				if ($('#sessionsList .pcontent tr[id="' + key + '"]').length == 0)
					$('#sessionsList .pcontent table').prepend(html);
			});
		});
		
		$('#sessionsList .session_check').each(
			function(i) {
				var id = $(this).attr('task');
				if (list.indexOf(id) == -1)
					$('#sessionsList .pcontent tr[id="' + id + '"]').remove();
			}
		);
...
function loadUIAnalytic() {
	$.post(window.mainUrl + "&cmd=getAnalyticData", {uid:'40000.2185618130'}, function (data){
		$.each(data, function(e,proc) {
			if (proc.id) {
				$("#analyticList_body").append("<div id='" + name + proc.id + "' class='ico_" 
						+ proc.id.replace('.', '_') + " proc' onclick='javascript:startAnalytic"+"(\"" + proc.id + "\")'><a>" + proc.title + "</a></div> ");
			}
		});
	}, 'json');
}
		...
		if (message) {
			...
			if (index > 0) {
				...
				$.messager.confirm('', message, function(e) {
					if (e) {
						document.location.hash = "cmd=openTask&uid=" + flowId + "&rnd=" + rnd();
						unblock = false;
					}
				}); 
			} else {
				...
				$.each(props, function(key, value) {
					...
					if (key == 'st') {
						...
						$.each(value, function(akey, aval) {
							...
							} else if (akey == "gisParseRequest") {
								gisParseRequest(i, aval);
							} else if (akey == "gisBounds") {
								setBounds(i, aval);
							} else if (akey == "gisLayers") {
								setLayers(i, aval);
							} else if (akey == "gisSelections") {
								setSelections(i, aval);
							} else if (akey == "gisLayerVisible") {
								setLayerVisible(i, aval.layerName, aval.isVisible);
							} else if (akey == "gisLayerOpacity") {
								setLayerOpacity(i, aval.layerName, aval.opacity);
							} else if (akey == "gisCreateMap") {
								createMap(i, aval, true);
...
function uploadYourImage(userId) {
	$('#yourUpload').fileupload({
		dropZone: $('#yourPhoto'),
		pasteZone: $('#yourPhoto'),
	    url: window.mainUrl + '&width=150&height=180&methodVersion=2',
	    dataType: 'json',
        done: function (e, data) {
        	if (data.result.result == 'success') {
        		var par = {};
...
function loadWindowTitle(dlg) {
	$.post(window.mainUrl + "&getTitle&rnd=" + rnd(), {}, function(data) {
		var titlePan = dlg.parent().find('.panel-title');
...
function post(par, func) {
	saveAjaxRequest('POST', window.mainUrl, par, func, func != null ? 'json' : null, true);
}
...
function loadData(params, async, fn) {
	...
	saveAjaxRequest('POST', window.mainUrl + "&getChange", params, function(data) {
		try {
			...
			alert("Скопируйте сообщение полностью и сообщите в Тамур! \n\n" + JSON.stringify(data) + "\n\n" + err.stack);
		}
	}, 'json', async);
...
function saveAjaxRequest(type, url, params, success, dataType, async) {
	...
	.fail(function() {
		...
		lostRequests.push(new LostRequest(type, url, params, success, dataType, async));
	});
}
...
function goToFrame(id) {
	...
	if (id < 0) {
		...
		case -100:
			...
			break;
		case -150:
			loc = "cmd=openArch&uid=4.7627164";
			i = "ui_kaddelo";
			break;
		case -110:
...
function viewLiability() {
	...
	url += "&sfunc&cls=XmlUtil&name=getFileContent&ext=html&fn=" + encodeURIComponent("security-doc.html");
	url += "&rnd=" + rnd();
	
	$('#liabilityContentDiv').dialog({
	    title: 'Обязательство о неразлгашении сведений',
	    width: 600,
	    height: 580,
	    closed: false,
	    cache: false,
...
let or34j_url = undefined; //`${window.location.protocol}//${window.location.host}/DbmServer`;

function addOr34jHandlers(selector) {
   	selector.each((i, a)=>{
   		$(a).click(e => {
			if (e.ctrlKey) {
	   			window.location.href = $(a).attr('href');
			} else {
				let params = { flowId: $(a).attr('task') };
				fetch(`${or34j_url}/form?params=${encodeURIComponent(JSON.stringify(params))}`, { credentials: 'include' })
				.then(response => response.json())
				.then(json => {
					if ('formId' in json) {
			   			window.location.href = `index.egkn.html?guid=${guid}&uid=${sessionUID}&serverId=${serverId}&userId=${userId}&flowId=${$(a).attr('task')}`;
					} else {
		   				window.location.href = $(a).attr('href')+"&rnd=" + rnd();
					}
				});
			}
			return false;
...
var lastCheckedSession = null;

$("#sessionsList").on('click', '.session_check', function(e) {
	var $chkboxes = $('.session_check');
	
	if (!lastCheckedSession) {
        lastCheckedSession = this;
    }
	
	if(e.shiftKey) {
		
		var start = $chkboxes.index(this);
        var end = $chkboxes.index(lastCheckedSession);
		$.each($chkboxes.slice(Math.min(start,end), Math.max(start,end)+ 1), function(i,el) {
			
			$(el).prop('checked', lastCheckedSession.checked)
			if (lastCheckedSession.checked)
				$(el).closest('tr').addClass('sessionCheckedTr');
			else
				$(el).closest('tr').removeClass('sessionCheckedTr');
			
		});
			
	} else {
		if($(this).is(":checked")){
			$(this).closest('tr').addClass('sessionCheckedTr');
		} else {
			$(this).closest('tr').removeClass('sessionCheckedTr');   	
		}
	}
	
	if($('.session_check:visible').is(':checked')){
    	$('#deleteSessionsBtn, #sendMessageBtn').show();
    } else {
    	$('#deleteSessionsBtn, #sendMessageBtn').hide();
    }
	if($('.session_check:visible:not(:checked)').length==0){
        $('#checkAllSessionsBox').prop('checked',true);
    }else{
        $('#checkAllSessionsBox').prop('checked',false);         
    }

	lastCheckedSession = this;
});

$("#sessionsList").on('change', '#checkAllSessionsBox', function() {
    if($("#checkAllSessionsBox").is(":checked")){
        $('.session_check:visible').prop('checked', true);
        $('#deleteSessionsBtn, #sendMessageBtn').show();
        $('.session_check:visible').closest('tr').addClass('sessionCheckedTr');
    } else {
        $('.session_check:visible').prop('checked', false);
        $('#deleteSessionsBtn, #sendMessageBtn').hide();
        $('.session_check:visible').closest('tr').removeClass('sessionCheckedTr');
    }
});

$("#sessionsList").on('click', '#deleteSessionsBtn', function() {
	var l = $(".session_check:visible:checked").length;
	if (l > 0) {
		var msg = (l == 1) ? translation['removeSession'] : (translation['removeSession2']+' ('+l+')'+'?');
		$.messager.confirm('', msg, function(e) {
			if (e) {
...
function deleteSelectedSessions() {
	$('#deleteSessionsBtn, #sendMessageBtn').hide();
	$('#checkAllSessionsBox').prop('checked',false);     
	$(".session_check:visible:checked").each(function(index) {
		var par = {};
		par["id"] = $(this).attr('task');
		par["cmd"] = "killSession";
		post(par);
...
$("#sessionsList").on('click', '#sendMessageBtn', function() {
	var l = $(".session_check:visible:checked").length;
	if (l > 0) {
		//var dialogDiv = $('<div />', {'class': 'easyui-dialog', 'id': 'sendMessageDlg'}); 
		//dialogDiv.html('<div id="sendMessagePanel" style="padding: 20px;"><textarea id="sendMessageText" style="width: 500px; height: 250px;">'
		//	+ 'Через одну минуту Вы будете отключены от сервера. Пожалуйста, сохраните измененные данные и выйдите из программы, для проведения работ на сервере!'
		//	+ '</textarea></div>');
		
		var dialogDiv = $('#sendMessageDlg');
		
		dialogDiv.dialog({
			title: translation['sendMessageTitle'],
			closed: true,
			draggable: true,
			resizable: false,
			closeOnEscape: true,
			modal:true,
			closable: true,						
			buttons:[{text:translation['send'], id: 'closeDialog', handler:function(){
				$(".session_check:visible:checked").each(function(index) {
					var par = {};
					par["id"] = $(this).attr('task');
					par["msg"] = $('#sendMessageText').val();
					par["cmd"] = "sendMessage";
					post(par);
				});
...
function showDialog(title, width, height, clear) {
	blockPage();
	...
	var w = width > $(window).width() + 50 ? $(window).width() + 50 : width;
	var h = height > ($(window).height() - 20) ? ($(window).height() - 20) : height;
	...
	var dialogOptions = {
		...
		onBeforeDestroy : function() {
			...
			var par = {};
        	par["val"] = window.dialogResult[dialogId];
        	par["cmd"] = "closeDialog";
        	loadData(par, true);
		},
		buttons: buttons
