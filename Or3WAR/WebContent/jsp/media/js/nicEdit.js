	...
	getStyle : function(cssRule, d) {
		...
		if (this.nodeType == 1)
			return (doc && doc.getComputedStyle) ? doc.getComputedStyle(this,
					null).getPropertyValue(cssRule) : this.currentStyle[bkLib
					.camelize(cssRule)];
	},
...
var bkLib = {
	...
	addEvent : function(obj, type, fn) {
		(obj.addEventListener) ? obj.addEventListener(type, fn, false) : obj
	...
	onDomLoaded : function(fireThis) {
		...
		if (document.addEventListener) {
			...
			document
					.write("<style>.nicEdit-main p { margin: 0; }</style><scr"
...
var nicEditorConfig = bkClass.extend({
	...
	buttonList : [ 'save', 'bold', 'italic', 'underline', 'left', 'center', 'right', 'justify', 'ol', 'ul', 'fontSize', 'fontFamily', 'indent', 'outdent', 'image', 'upload', 'link', 'unlink', 'forecolor', 'bgcolor', 'table' ],
	iconList : {
		"bgcolor" : 1,
		"forecolor" : 2,
		"bold" : 3,
...
var nicEditors = {
	...
	allTextAreas : function(nicOptions) {
		...
		for ( var i = 0; i < textareas.length; i++) {
			nicEditors.editors.push(new nicEditor(nicOptions)
					.panelInstance(textareas[i]));
		}
...
var nicEditor = bkClass.extend({
	...
	panelInstance : function(e, o) {
		...
		// добавить ID для панели инструментов
		$(panelElm).attr("id", $(e).attr('id')+'_ed');
	...
	getIcon : function(iconName, options) {
		...
		return {
			background : iconName == "table" ? "no-repeat" : "",
			backgroundImage : "url('" + ((icon) ? this.options.iconsPath : file) + "')",
			backgroundPosition : iconName == "table" ? "center" : ((icon) ? ((icon - 1) * -18) : 0) + 'px 0px'
		};
	},
		...
		.extend({
			...
			construct : function(e, options, nicEditor) {
				...
				if (isTextarea || this.options.hasPanel) {
					...
					s[(ie7s) ? 'height' : 'maxHeight'] = (this.ne.options.maxHeight) ? this.ne.options.maxHeight
							+ 'px'
							: null;
					this.editorContain = new bkElement('DIV').setStyle(s)
							.appendBefore(e);
					...
					$(block).css('margin', '4px');
					$(block).css('width', newX - 8 + 'px');
					$(block).css('min-height', newY - 8 + 'px');
					$(block).css('z-index', -1);
					...
					$(editorElm).attr("id", $(e).attr('id'));
					func = $(e).attr('onchange');
				...
				if (r.startContainer) {
					...
					if (r.cloneContents().childNodes.length == 1) {
						for ( var i = 0; i < contain.childNodes.length; i++) {
							var rng = contain.childNodes[i].ownerDocument
									.createRange();
							rng.selectNode(contain.childNodes[i]);
							if (r.compareBoundaryPoints(Range.START_TO_START,
									rng) != 1
									&& r.compareBoundaryPoints(
											Range.END_TO_END, rng) != -1) {
			...
			blur : function() {
				if (this.isFocused) {
					...
					if (this.copyElm.innerHTML != this.elm.innerHTML) {
						this.copyElm.innerHTML = this.elm.innerHTML;
						$(this.copyElm).change();
					}
				}
			},
			...
			nicCommand : function(cmd, args) {
				if (cmd == 'fontsize' || cmd == 'fontname' || cmd == 'foreColor') {
					pasteHtmlAtCaret(cmd, args);
				} else {
			...
			construct : function(e, buttonName, options, nicEditor) {
				...
				this.button = new bkElement('DIV').setStyle({
					...
					cursor : 'pointer'
						...
						this.ne.getIcon(buttonName, options)).appendTo(
						this.border);
				...
				nicEditor.addEvent('selected', this.enable.closure(this))
						.addEvent('blur', this.disable.closure(this)).addEvent(
								'key', this.key.closure(this));
			...
			checkNodes : function(e) {
				...
				do {
					if (this.options.tags
							&& bkLib.inArray(this.options.tags, elm.nodeName)) {
						this.activate();
						return true;
					}
		...
		if (openButton && !openButton.options.noClose) {
			this.close = new bkElement('div').setStyle({
				...
				cursor : 'pointer'
					...
					'mousedown', openButton.removePane.closure(this)).appendTo(
					this.pane);
		}
			...
			if (type == 'title') {
				...
				case 'select':
					...
					for (opt in field.options) {
						...
						}).setContent(field.options[opt]).appendTo(
								this.inputs[itm]);
					}
					break;
				case 'content':
		...
		.extend({
			...
			open : function() {
				...
				for ( var i = 0; i < this.selOptions.length; i++) {
					...
					itm
							.addEvent('click', this.update.closure(this, opt[0]))
...
var nicEditorFontSizeSelect = nicEditorSelect.extend({
	...
	init : function() {
		...
		for (itm in this.sel) {
			this.add(itm, "<span style='font-size:" + this.sel[itm] + ";'>" + this.sel[itm] + '</span>');
		}
	}
});
...
var nicEditorColorButton = nicEditorAdvancedButton
		.extend({
			addPane : function() {
				var colorList = {
					0 : '00',
						...
						for ( var g in colorList) {
							...
							var colorInner = new bkElement('DIV').setStyle({
								...
								height : '11px'
									...
									this.colorSelect.closure(this, colorCode))
									.addEvent('mouseover',
											this.on.closure(this, colorBorder))
									.addEvent(
											'mouseout',
											this.off.closure(this, colorBorder,
													colorCode)).appendTo(
											colorBorder);
...
function pasteHtmlAtCaret(cmd, args) {
	var range = window.getSelection().getRangeAt(0);
	var selectionContents = range.extractContents();
	var span = document.createElement("span");
	span.style.display = 'inline'; 
	if (cmd=='fontsize'){
		span.style.fontSize = args;
	}else if(cmd=='fontname'){
		span.style.fontFamily = args;
	}else if(cmd=='foreColor'){
		span.style.color = args;
	}
...
var nicTableButton = nicEditorAdvancedButton.extend({
	...
	submit: function (e) {
		...
		var tableCode = "<table width='" + width +"' style='border-collapse: collapse; border: 1px solid black;'><thead><tr>";

		for (i = 1; i <= cols; i++) {
			tableCode += "<th width='" + cellw + "%' style='border: 1px solid black;'>Заголовок" + i + "</th>";
		}
		...
		for (j = 1; j <= rows; j++) {
			tableCode += '<tr>';
			for (i = 1; i <= cols; i++) {
				tableCode += "<td width='" + cellw + "%' class='" + alternate + "' style='border: 1px solid black;'>Содержимое" + j + "</td>";
			}
