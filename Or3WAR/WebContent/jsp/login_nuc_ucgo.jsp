...
<link rel=" stylesheet" type="text/css" href="<%=pathCSS%>uiv1.css">
...
<div id="loginbox">
	...
	<div id='tt' class="easyui-tabs" data-options="plain:true, narrow:true">
		<div title="Логин/Пароль">
			<img src="<%=webContextName%>/main?loginLogo" width="222" height="80">
		...
		<div title="ЭЦП">
			...
			<form id="loginECPForm" onsubmit="return loginToSystem();">
			    <div id="loginECPType">
			        <input type="radio" name="ecp" value="1" style="width:15px;height:15px;margin:10px;" checked="checked"><label style="font-family:Tahoma;font-size:12px;">НУЦ</label>
			        <input type="radio" name="ecp" value="2" style="width:15px;height:15px;margin:10px;"><label style="font-family:Tahoma;font-size:12px;">УЦГО</label>
		    	</div>
				<input type="text" name="file" value="" class="input" id="file" placeholder="Ключевой контейнер" style="width: 230px;" />
				<input type="button" name="sel" value="..." class="button" id="selFile" onclick="selectFile(document.getElementById('file').value);" style="width: 30px; padding: 0; position:absolute;" />
	...
	<div class="btnPanel">
		<button id="loginBtn" class="btn btn-primary" onclick="loginFunc(false)" style="font-size: 16px; padding: 4px 10px;">Войти в систему</button>
		<button id="loginECPBtn" class="btn btn-primary" onclick="loginToSystem()" style="font-size: 16px; padding: 4px 10px;">Войти в систему</button>
		<button id="resetBtn" class="btn btn-primary" onclick="reset()" style="font-size: 16px; padding: 4px 10px;">Очистить</button>
...
<script type='text/javascript' src='<%=pathJS%>ncaLayerOps.js?v=2018-08-08'></script>
<script type='text/javascript' src='<%=pathJS%>liability.dialog.js?v=2018-08-08'></script>
<%-- <script type='text/javascript' src='<%=pathJS%>app.js'></script> --%>
<script type='text/javascript' src='<%=pathJS%>tumsocket_or3.js'></script>
<script>
...
var rad = document.getElementsByName('ecp');
var prev = null;
for (var i = 0; i < rad.length; i++) {
    rad[i].addEventListener('change', function() {
    	loginECPType = this.value;
    	if (loginECPType == 1) {
    		document.getElementById("file").disabled = false;
    		document.getElementById("selFile").disabled = false;
    		document.getElementById("pdECP").disabled = false;
// 			$('#file').show();
// 			$('#selFile').show();
// 			$('#pdECP').show();
        } else {
...
// 			$('#pdECP').hide();
    		...
    		document.getElementById("pdECP").disabled = true;
...
function loginWithApplet(secret) {
	try {
		var s = document.getElementById('ECPApplet').getSignAndCert(secret, 17, $('#file').val(), $('#pdECP').val(), true);
		if (s != null) {
				...
				$.post(url, par, function(data) {
					if (data.result == "success") {
						...
						if (data.la) {
							...
							window.location.href = "<%=webContextName%>/jsp/index.jsp?guid=" + guid + "&rnd=" + rnd();
						}
					} else {
						alert(data.message);
						$('#loginECPBtn').removeClass("btn-disabled").removeAttr("disabled").html('Войти в систему');
	...
	if (!$('#loginECPBtn').hasClass('btn-disabled')) {
		...
		$.post(url, {cmd:'getSecret', rnd: rnd(), guid: guid, json:1}, function(data) {
			if (data.secret) {
				secret = data.secret;
				if (navigator.javaEnabled()) {			
					loginWithApplet(secret);
				} else {
					if (loginECPType == 1) {
						mode = 0;
						getKeysCall('getKeysBack');
					} else {
						generateUcgoSign(secret, null, null, null, true);
					}
				}
			} else {
				alert(data.message);
...
function post(params, func) {
	params["rnd"] = rnd();
	params["guid"] = guid;
	$.post(url, params, func, func != null ? 'json' : null);
}
...
function loginWithUCGO(data, params) {
	var par = {};
	par["json"] = 1;
	par["isUCGO"] = 1;
	par["browser"] = browser;
