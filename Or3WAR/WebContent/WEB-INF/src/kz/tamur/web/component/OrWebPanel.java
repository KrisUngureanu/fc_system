 ...
 */
public class OrWebPanel extends WebPanel implements OrPanelComponent {
    ...
    private ASTStart beforeOpenTemplate, afterOpenTemplate, beforeCloseTemplate, afterCloseTemplate, createXmlTemplate, afterSaveTemplate,
    					afterTaskListUpdateTemplate, onNotificationTemplate, messageRecievedTemplate;
    private String borderTitleUID;
    private String titleAlign = "center";
    private boolean isHideBreadCrumps;
    private boolean isDataIntegrityControl;

    public OrWebPanel(Element xml, int mode, WebFactory fm, OrFrame frame, String id) throws KrnException {
    ...
    private void init(WebFactory fm, int mode) throws KrnException {
        setLayout(gbl);
        adapter = new PanelAdapter(frame, this, false);

        PropertyNode pn;
        PropertyValue pv;
        if (frame.getKernel().isRNDB()) {
        ...
        if (mode == Mode.RUNTIME) {
            ...
            if (pn != null) {
                ...
                if (expr != null && expr.length() > 0) {
                    messageRecievedTemplate = OrLang.createStaticTemplate(expr, getLog());
                }
            }
            ...
            pv = getPropertyValue(PROPS.getChild("constraints").getChild("dataIntegrityControl"));
            if (!pv.isNull()) {
            	isDataIntegrityControl = pv.booleanValue();
            }
        }
    }
    ...
    public boolean isDataIntegrityControl() {
    	return isDataIntegrityControl;
    }
    ...
    private void updateProperties() {
        ...
        } else if (!Constants.removeDefaultColor) {
