...
public class OrRef implements OrRefListener, CashChangeListener,
    ...
    public List<Item> getSelectedItems() {
        ...
        for (int i = 0; !contains && selectedInds_ != null && i < selectedInds_.length; i++) {
    ...
    public int[] getPreviousSelectedIndexes() {
        List<Integer> res = new ArrayList<Integer>();
        if ((selectedInds_ == null || selectedInds_.length == 0) && index > -1)
            selectedInds_ = new int[] { index };
	...
	public void setSelectedItems(KrnObject[] objs) {
		...
		for (KrnObject obj : objs) {
			...
			// Сначала проверяем выделенный объект
			KrnObject curr = (KrnObject) values.get(0, index).getCurrent();
			if (curr != null && obj.id == curr.id) {
				selInds.add(index);
				isFound = true;
			}

			// Если не совпадает, то ищем среди всех значений
			if (!isFound) {
				for (int i = 0; i < values.size(0); ++i) {
					curr = (KrnObject) values.get(0, i).getCurrent();
					if (curr != null && obj.id == curr.id) {
						selInds.add(i);
						isFound = true;
						break;
		...
		if (mode == Mode.RUNTIME && path != null) {
			...
			if (count > 0) {
				...
				for (int i = 0; i < count - 1; ++i) {
					...
					if (attr_ == null) {
						/*
						 * log.info("ERROR: Attribute '" + aname + "'
						 * not found"); log.info(" in path '" + path +
						 * "'");
						 */
				...
				if (attr_ != null) {
					getCash().addCashChangeListener(attr_.id, this, frm);
			...
			if (!allowsLangs) {
				...
				KrnObject lang = isContent ? frm.getInterfaceLang() : frm.getDataLang();
				if (lang != null)
					setLangId(lang.id);
			}
		} else
	...
	private OrRef(String path, boolean isColumn, int mode, boolean isContent,
		...
		if (mode == Mode.RUNTIME && path != null) {
			PathElement2[] ps = Utils.parsePath2(path, krn_);
			int count = ps.length;
			if (count > 0) {
				...
				refs.put(Utils.normalizePath(path).intern(), this);
				if (attr_ != null) {
					getCash().addCashChangeListener(attr_.id, this, frm);
	...
	public int preEvaluate(KrnObject[] objs, Object originator)
		...
		if (objs == null && cls_ != null) {
			int[] ih = {limit};
			recs = cash.getObjects(cls_.id, defaultFilterId_, ih, isLoaded_, this);
    ...
    public void evaluate(KrnObject[] objs, Object originator, boolean sort)
        ...
        try {
			...
			if (objs == null && cls_ != null) {
				int[] ih = {limit};
	            Set<ObjectRecord> recs = defaultFilterId_ > 0 ? new LinkedHashSet<ObjectRecord>() :	new TreeSet<ObjectRecord>();
	            AttrRequest req = getRequest(null);
	            if (req.getChildren().size() > 0)
	            	recs.addAll(cash.getObjects(cls_.id, req, defaultFilterId_, ih, isLoaded_ || ((WebFrame)frm).isSharedCache(), this));
	            else
	            	recs.addAll(cash.getObjects(cls_.id, defaultFilterId_, ih, isLoaded_ || ((WebFrame)frm).isSharedCache(), this));
	            if (recs.size() > 100) {
	            	log.info("Предупреждение: Отбор " + recs.size() + " записей '" + this + "'.");
	            }
	            if (ErrorsNotification.isInitialize() && recs.size() > ErrorsNotification.getMaxRecords()) {
	            	WebFrame wframe=(WebFrame)frm;
	            	WebSession wsession=wframe.getSession();
	            	// TODO Добавить данные по интерфейсу 
	            	String message="Предупреждение: Отбор " + recs.size() 
	            			+ " записей '" + this + "' пользователь:"+wsession.getUserName()+".";
	            	ErrorsNotification.notifyErrors("TO_300", "INTERFACE", message, null,null);
	            }
				limitExceeded = ih[0] > limit;
	            ...
	            for (Record rec : recs) {
	            	if (deletedAttr_ != null && rec.getValue() instanceof KrnObject) {
	        			...
	        			if (dr != null && Long.valueOf(1).equals(dr.getValue()))
		...
		if (items != null && items.size() > itemIndex) {
			...
			if (item != null) {
				...
				if (isColumn) {
					item.rec = null;
					try {
						if (parent_ != null && isArray()) {
							Item pitem = parent_.getItem(0, itemIndex);
							if (pitem.getCurrent() instanceof KrnObject) {
								long id = ((KrnObject)pitem.getCurrent()).id;
								SortedSet<Record> recs = cash.getRecords(new long[] {id}, attr_, lid, null);
								Record rec = getLast(id, lid, recs, attrIndex_);
								item.rec = rec;
							}
						}
					} catch (Exception e) {
	...
	public void deleteItem(
		...
		if (obj == null) return;
		int itemIndex = indexOfObjectItem(obj.id);
		if (itemIndex < 0) return;
	...
	public void refresh(Object originator) throws KrnException {
		if (parent_ != null)
			evaluate(parent_, originator, 0);
		else
			evaluate(objs_, originator);
	}
	...
	private void preloadCache(long[] pids) {
		...
		if (filterIds_.size() > 0 || defaultFilterId_ > 0) // необходимость фильтрации на компаненте
			return;
	...
	public void checkReqGroups(OrRef ref, List<ReqMsgsList.MsgListItem> msgs,
		...
		for (int i = 0; i < checkContextList.size(); i++) {
			...
			if (!context.isActive() && (frm.getKernel().isRNDB() || (!(context instanceof TreeTableAdapter2) && !(context instanceof TreeTableAdapter) && !(context instanceof TableAdapter)))) {
				if (!listenersChecked) {
			...
			if (isInOrTable() && context.isCheckConstr()) {
				...
				// tableAdapter.getTable().getProperties().getChild("title")).stringValue(frame.getKernel());
				if (!listenersChecked) {
					listenersChecked = true;
...
//			CheckContext context = (CheckContext) checkContextList.get(i);
			...
			if (o != null && !"".equals(o) && ctemplate != null) {
				try {
                	...
                	log.error("Ошибка при выполнении формулы 'Ограничение', uuid: " + context.getUUID());
                    log.error(e, e);
					...
					if (!isOk) {
						if (error == null) {
							...
							if (constrMsg != null) {
								...
								log.warn("!ERROR: Не задано сообщения для "
											+ "ограничения '"
											+ context.getCExpr() + "'");
							}
						}
...
//		}
	...
	public void pathChanged(OrRefEvent e) {
		...
		if (title_ != null && title_.length() > 0) {
			try {
				...
				KrnAttribute[] attrs = Utils.getAttributesForPath(title_, krn_);
	...
	//Уведомляет слушателей (адаптеры)
...
//			if (exitPrevFire) break;
			...
			for (long langId : langIds) {
				...
				for (int i = startIndex; i < endIndex; ++i) {
					...
					if (pobj != null) {
		            	...
		            	if (deletedAttr_ != null && rec.getValue() instanceof KrnObject) {
		        			Record dr = cache.getRecord(((KrnObject)rec.getValue()).id, deletedAttr_, 0, 0);
		        			if (dr != null && Long.valueOf(1).equals(dr.getValue()))
		        				newItem = new Item(null, rec, true, true);
		        			else
		        				newItem = new Item(null, rec, true, false);
			...
			if (pitem != null && pitem.getCurrent() instanceof KrnObject) {
				...
				for (long langId : langIds) {
					...
					if (attrIndex_ == null || attrIndex_ instanceof String) {
                        if (recs.size() == 0 && (childrenAttr_ != null || childrenExpr_ != null))
                            rootItem = null;
	...
	public class Item implements Comparable<Item> {
		...
		public List<Item> getChildren(CheckContext ctx, int[] limit) throws KrnException {
			...
			if (children == null) {
				...
				if (o instanceof KrnObject) {
					...
					if (childrenExpr_ != null) {
				        ...
				        vc.put("OBJ", o);
				        try {
				            lng.evaluate(childrenExpr_, vc, ctx, new Stack<String>());
				        } catch (Exception e) {
				        	log.error("Ошибка при считывании детей в дереве uuid: " + ctx.getUUID());
				        	log.error(e, e);
				        } finally {
				        	if (oldFrame != null)
				        ...
				        if (res instanceof List) {
				        	long[] oids = null;
				        	if (defaultFilterId_ > 0) {
								oids = getFilteredObjects();
				        	}
				        	List<KrnObject> finalRes = new ArrayList<KrnObject>();
				        	for (KrnObject obj : (List<KrnObject>)res) {
				        		if (obj != null && (obj.id < 0 || oids == null || Funcs.contains(oids, obj.id))) {
				        			finalRes.add(obj);
				        		}
				        	}
							...
							AttrRequest req = getRequest(null);
							if (req.getChildren().size() > 0) {
								long[] objIds = Funcs.makeObjectIdArray(finalRes);
								cache.getObjects(objIds, req);
							}
				        	for (KrnObject obj : finalRes) {
						...
						SortedSet<Record> recs = cache.getRecords(
								objIds, childrenAttr_, 0, filterIds, lostIds, limit, null, true, req);
	...
	public void release() {
		isCleared = true;
		Object[] listeners = listeners_.getListenerList();
		List<OrRefListener> t = new ArrayList<OrRefListener>();
		for (int i = listeners.length - 2; i >= 0; i -= 2) {
			if (listeners[i] == OrRefListener.class) {
				OrRefListener l = (OrRefListener) listeners[i + 1];
				t.add(l);
		...
		for (OrRefListener l : t) {
			removeOrRefListener(l);
		}
		t.clear();
...
*/
    ...
    private int indexOfObjectItem(long objId) {
		List<Item> items = getItems(0);
		for (int i = 0; i < items.size(); i++) {
			Object obj = items.get(i).getCurrent();
			if (obj == null || obj instanceof KrnObject) {
				KrnObject curr = (KrnObject) obj;
				if (curr != null && objId == curr.id) {
					return i;
				}
			} else
    ...
    private AttrRequest getRequest(AttrRequest preq) {
    	...
    	if (preq == null
    		...
    		if (preq == null) {
	    		...
	    		if (children_ != null && children_.size() > 0) {
					List<OrRef> nchildren = new ArrayList<OrRef>(children_);
					Collections.sort(nchildren, new Comparator<OrRef>() {
						public int compare(OrRef r1, OrRef r2) {
							KrnAttribute a1 = r1.getAttr();
							KrnAttribute a2 = r2.getAttr();
							long res = (a1 != null ? a1.classId : 0) - (a2 != null ? a2.classId : 0);
							...
							try {
								boolean sub = krn_.isSubclassOf(a1.classId, a2.classId);
								if (sub) return 1;
							} catch (Exception e) {}
	    		...
	    		if (hasChildrenAttr_ != null) {
	    			boolean hasHasChildrenAttr_ = false;
	    			List<AttrRequest> chs = req.getChildren();
	    			for (AttrRequest ch : chs) {
	    				if (ch.attrId == hasChildrenAttr_.id) {
	    					hasHasChildrenAttr_ = true;
	    					break;
	    				}
	    			}
	    			if (!hasHasChildrenAttr_) {
		    			AttrRequest hcReq = new AttrRequest(req);
		    			hcReq.attrId = hasChildrenAttr_.id;
	    			}
	    		}
    ...
    protected void initLog(Kernel krn) {
    ...
    private long[] getFilteredObjects() throws KrnException {
    	if (filteredObjs_ == null && defaultFilterId_ > 0) {
    		filteredObjs_ = krn_.getFilteredObjectIds(new long[] { defaultFilterId_ }, 
					new FilterDate[0], new int[] {limit}, getCash().getTransactionId());
    	}
    	return filteredObjs_;
    }
