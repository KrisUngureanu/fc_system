...
<link rel="stylesheet" type="text/css" href="<%=pathCSS%>docflow.css">
<link rel="stylesheet" type="text/css" href="<%=pathCSS%>uiv<%=theme%>.css?2020-11-23">
...
<% } %>
			...
			<ul
				...
				<%
					...
					if(navBarComps != null && navBarComps.size() > 0) {
						for (int kk = 0; kk < navBarComps.size(); kk++) {
							NavBarComponent comp = navBarComps.get(kk);
							if ("ui_personInfo".equals(comp.getAid()))
								navBarComps.set(kk, new NavBarComponent(60, null, "ui_personInfo", "#cmd=openArch&uid=1014162.3198690&id=ui_personInfo", Funcs.sanitizeXml(rb.getString("webStartPage")), false, null, null, -50));
						}
						for (NavBarComponent comp : navBarComps) {
							WebController.jspLog.warn("comp name = " + comp.getAid() + ", i = " + mm++);
							if(comp.isDynamic()){
	...
	<script type="text/javascript">
		...
		pageName['ui_personInfo'] = '<%=Funcs.sanitizeXml(rb.getString("webStartPage"))%>';
	...
	<script type="text/javascript" src="<%=pathJS%>app.js?v=2020-12-28"></script>
	...
	<script type="text/javascript" src="<%= pathJS %>tumsocket_or3.js?v=2021-04-02"></script>
