...
var win=dlg.dialog("dialog").addClass("messager-window").css("z-index",$.messager.defaults.zIndex++);;
...
var val=parseFloat(s).toFixed(c);
if(isNaN(val)){
val="";
}else{
...
if (p.parent() && p.parent().find('.treetable-search').length > 0) {
	opts.height=p.parent().height() - p.parent().find('.treetable-search').parent().height();
	p.parent().height(opts.height);
...
var td=$("<td rowspan=\""+opts.frozenColumns.length+"\"><div class=\"datagrid-header-rownumber\">№<br>"+opts.orderNumber+"</div></td>");
if($("tr",t).length==0){
...
if(!opts.nowrap){
_7d0+="white-space:normal;height:auto;overflow:unset;";
}else{
...
if(!opts.nowrap){
_943+="white-space:normal;height:auto;overflow:unset;";
}else{
if(opts.autoRowHeight){
_943+="height:auto;";
}
...
if(!_9b3.nopanel && _9b3.closed){
_9b2.panel("panel").show().css({zIndex:($.fn.menu?$.fn.menu.defaults.zIndex++:($.fn.window?$.fn.combo.defaults.zIndex++:99)),left:-999999});
...
for (si in s) {
	if ((typeof s[si] == "string" && s[si].toLowerCase() == q.toLowerCase())
	 	|| (s[si].title && s[si].title.toLowerCase() == q.toLowerCase())) {
		_a03=v;
		_9e6(_9ff,v,true);
		break;
	}
...
var tt=[_acf(date!=null?date.getHours():cur_hour),_acf(date!=null?date.getMinutes():cur_min)];
if(opts.showSeconds){
tt.push(_acf(date!=null?date.getSeconds():0));
}
