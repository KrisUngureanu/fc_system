...
<%
	...
	*/
    ...
    if (userUID == null) {
        ...
        boolean hasPerson = (s.get("hasPerson") != null) ? ((Boolean) s.get("hasPerson")).booleanValue() : false;

        Long tmp = (Long) s.get("userId");
        long userId = tmp != null ? tmp : 0;
        
        String sessionUID = ws.getKernel().getUUID().toString();
        ...
        String userLogin = (String) s.get("userLogin");
        if (userLogin == null) {
        	userLogin = "";
        }

        String serverId = (String) s.get("serverId");
        if (serverId == null) {
        	serverId = "";
        }
...
//        m.put("menu.main",true);
        ...
        String logoImg = "<img src='" + (logoPath == null ? "media/img/logo-egkn.png" : logoPath) + "'/>";
    	...
    	String gisServerUrl = getServletConfig().getServletContext().getInitParameter("gis.server.url");
...
<link href="<%=webContextName%>/jsp/media/map/static/css/main.chunk.css" rel="stylesheet">
...
<link rel="stylesheet" type="text/css" href="<%=pathCSS%>ol.css">
<link rel="stylesheet" type="text/css" href="<%=pathCSS%>uiv<%=theme%>.css?v=2020-12-08">
<link rel="stylesheet" type="text/css" href="<%=pathCSS%>or3v<%=theme%>.css?v=2020-12-08">
...
<link rel="stylesheet" type="text/css" href="<%=pathCSS%>ul.css?v=2020-12-18">
<% if (activateChat) { %>
...
<script type="text/javascript">
	...
	var selSect = <%out.print("'");%><%= (rb != null) ? Funcs.sanitizeXml(rb.getString("webNotification")) : "" %><%out.print("'");%>;
	// хранит ID раздела 
	var idSect=-10;
	
	var gisServerUrl = '<%= gisServerUrl %>';
...
<script type="text/javascript" src="<%=pathJS%>datagrid-cellediting.js?v=2021-10-25"></script>
...
<script type="text/javascript" src="<%=pathJS%>notifications.js?v=2020-11-24"></script>
<script type="text/javascript" src="<%=pathJS%>jodit.min.js"></script>
<script type="text/javascript" src="<%=pathJS%>or34j.js"></script>
</head>
...
<% if (showServiceDesk) { %>
				<td>
					<a id="helpPhone" class="hepl-icon" href="javascript:void(0)" title="" ></a>
				</td>							
<% } %>
			</tr>
		</table>
	...
	<nav id="left-panel"
		...
		<div id="left-panel-content" style="">
			<ul
				...
				<li <%if (startHash == null) {%> class="active" <%}%>><a
					onclick="forseSaveChanges(this); selSect='<%=Funcs.sanitizeXml(rb.getString("webNotification"))%>';idSect=-10;"
					href="#cmd=openDict&uid=40000.2184142131&id=ui_startPage" id="ui_startPage"><%=Funcs.sanitizeXml(rb.getString("webNotification"))%></a></li>
				<%
				    	if (startHash == null) startHash = "#cmd=openDict&uid=40000.2184142131&id=ui_startPage";
					}
				...
				<li <%if (startHash == null) {%> class="active" <%}%>><div
					...
					if (showMenuItem && hasPerson) {
				%>
				<li <%if (startHash == null) {%> class="active" <%}%>><a
					onclick="forseSaveChanges(this); selSect='<%=Funcs.sanitizeXml(rb.getString("webMyProfile"))%>';idSect=-50;"
					href="#cmd=openArch&uid=1014162.3198690&id=ui_personInfo" id="ui_personInfo"><%=Funcs.sanitizeXml(rb.getString("webMyProfile"))%></a></li>
				<%
				...
				<%
					...
					if (showMenuItem && Boolean.TRUE.equals(m.get("menu.kadmap"))) {
				%>
				<li <%if (startHash == null) {%> class="active" <%}%>><a
					onclick="if (event.ctrlKey) { $('#app').empty(); let params = {hiperTreeUid: '40000.2184146655'}; new or34j.App('http://192.168.13.67/DbmServer', document.getElementById('app'), new or34j.EasyFactory()).init(params).then(() => { initMap([]); }); return false;} else {forseSaveChanges(this); selSect='Кадастровая карта';idSect=-100;}"
					href="#cmd=openArch&uid=40000.2184146655&id=ui_stat" id="ui_stat">Кадастровая карта</a></li>
				<%
				    if (startHash == null) startHash = "#cmd=openArch&uid=40000.2184146655&id=ui_stat";
												}
					if (showMenuItem && Boolean.TRUE.equals(m.get("menu.kadwork"))) {
				%>
				<li <%if (startHash == null) {%> class="active" <%}%>><a
					onclick="forseSaveChanges(this); selSect='Кадастровое дело';idSect=-150;"
					href="#cmd=openArch&uid=4.7627164&id=ui_kaddelo" id="ui_kaddelo">Кадастровое дело</a></li>
				<%
				    if (startHash == null) startHash = "#cmd=openArch&uid=4.7627164&id=ui_kaddelo";
												}
				...
				%>
				<li <%if (startHash == null) {%> class="active" <%}%>><a
					onclick="forseSaveChanges(this); selSect='Аналитика';idSect=-160;"
					href="#ui=analyticList&mode=layout&id=ui_analytic" id="ui_analytic">Аналитика</a></li>
				<%
				    if (startHash == null) startHash = "#ui=analyticList&mode=layout&id=ui_analytic";
												}
				%>
			</ul>
		</div>
		...
		<div class="sysinfo server-ping">Сервер: <%= serverId %></div>
	...
	<div class="container">
			...
			<div data-options="region:'north',split:false,border:false" style="height: 115px;">
				...
				<div id="uiToolbar" class="ui-toolbar" style="display: none;">
					<div style="padding: 5px; display: inline;">
						...
						%>
						<a class="easyui-linkbutton"
							data-options="iconCls:'icon-next'" onclick="blockPage(); nextStep();"
		...
		%>	
		<div id="processesList" style="width: 100%; display: none;  box-sizing: border-box;" class='appwin'>
		...
		%>
		<div id="processesList" style="display: none; " class='appwin'>
			<table class="portlet-table" width="100%">
				<tr>
					<td>
		...
		%>
		<div id="archList"
			style="display: none; " class='appwin'>
			<table class="portlet-table" width="100%">
				<tr>
					<td>
				...
				<tr>
					<td>
						<div class="header"><%=Funcs.sanitizeXml(rb.getString("webMonitor"))%>
							...
							<% if (isAdmin) { %>
							<button id="reloadprocess" style="display: none;" class="easyui-linkbutton c5">Перезагрузить процесс</button>	
							<% } %>
		...
		%>
		<div id="notiInList" style="display: none; " class='appwin'>
			<table class="portlet-table" width="100%">
				<tr>
					<td>
				...
				<tr>
					<td>
						<div class="header"><%=Funcs.sanitizeXml(rb.getString("webSessions"))%>
							...
							<input type="checkbox" id="checkAllSessionsBox"/>
							<label style="margin-left: 5px;" title="<%= Funcs.sanitizeXml(rb.getString("webAllCheckedProc")) %>"><%= Funcs.sanitizeXml(rb.getString("webAllCheckedProc")) %></label>
							
							<button id="deleteSessionsBtn" style="display: none;" class="easyui-linkbutton c5">
								<font style="color:white;"><%= Funcs.sanitizeXml(rb.getString("webDeleteSessionBtn"))%></font>
							</button>
								
							<button id="sendMessageBtn" style="display: none;" class="easyui-linkbutton c1">
								<font style="color:white;"><%= Funcs.sanitizeXml(rb.getString("webSendMessageBtn"))%></font>
							</button>
										...
										<tr>
											<td width="50%">
												<div class="pr">
													...
													<div class="uRole prof"><%=Funcs.sanitizeXml(rb.getString("IIN"))%></div>
													<%=userIIN%>
													<div class="uRole prof"><%=Funcs.sanitizeXml(rb.getString("ipAddress"))%></div>
													<%=userIP%>
													<a href="javascript:viewLiability();">
													<div class="uRole prof">Документ о неразглашении сведений</div>
													</a>
													<%=Funcs.sanitizeXml(rb.getString("webLastSuccessfullTime"))%>: <%=s.get("lastSuccessfullTime")%><br/>
													<%=Funcs.sanitizeXml(rb.getString("webLastUnsuccessfullTime"))%>: <%=s.get("lastUnsuccessfullTime")%>
		...
		%>
		<div id="statWnd" style="display: none; "
			class='appwin'>
			<table class="portlet-table" width="100%">
				<tr>
					<td>
		...
		%>
		<div id="helpWnd" style="display: none; "
			class='appwin'></div>
		<%
		    }
						if (showMenuItem && Boolean.TRUE.equals(m.get("menu.statistics"))) {
		...
		<div id="analyticList"
			...
			<table class="portlet-table" width="100%">
				<tr>
					<td>
						<div class="header"><%=Funcs.sanitizeXml(rb.getString("olapAnalytic"))%></div>
					</td>
				</tr>
				<tr>
					<td>
						<div class="portlet">
							<div class="pcontent easyui-panel"
								data-options="fit:true,border:false">
								<div id="analyticList_Layout" class="easyui-layout"
									data-options="fit:true,border:false">
									<div data-options="region:'center',border:false"
										id="analyticList_body" style="padding: 5px; background: #fff;"></div>
								</div>
							</div>
						</div>
					</td>
	...
	<script type="text/javascript">
		...
		translation['mask2'] = '<%=Funcs.sanitizeXml(rb.getString("mask2"))%>';
		...
		translation['send'] = '<%=Funcs.sanitizeXml(rb.getString("send"))%>';

		translation['removeSession'] = '<%=Funcs.sanitizeXml(rb.getString("webRemoveSession"))%>';
		translation['removeSession2'] = '<%=Funcs.sanitizeXml(rb.getString("webRemoveSession2"))%>';
		translation['sendMessageTitle'] = '<%=Funcs.sanitizeXml(rb.getString("webSendMessageTitle"))%>';
		...
		pageName['ui_stat'] = 'Кадастровая карта';
		pageId['ui_stat'] = -100;

		pageName['ui_kaddelo'] = 'Кадастровое дело';
		pageId['ui_kaddelo'] = -150;
	...
	<script type="text/javascript" src="<%=pathJS%>app.gis.js?v=2023-05-24"></script>
	<script type="text/javascript" src="<%=pathJS%>map.js?v=2018-12-23"></script>
	<script type="text/javascript" src="<%=pathJS%>analytic.js?v=2021-09-27"></script>
	<script type="text/javascript" src="<%=pathJS%>ol-custom.js"></script>
	<script type="text/javascript" src="<%=pathJS%>grid.editors.js?v=2023-05-09"></script>
	<script type="text/javascript" src="<%=pathJS%>grid.formatter.js?v=2018-12-12"></script>
	<% if (activateChat) { %>
		...
		<script type="text/javascript" src="<%=webContextName%>/chat/js/chat.egkn.js?v=2019-12-24"></script>
	<% } %>
	<SCRIPT TYPE='text/javascript' SRC='<%= pathJS %>applet-load.js?v=2016-09-19'></SCRIPT>
	...
	<script src="<%=webContextName%>/jsp/media/map/index.js"></script>
	<script src="<%=webContextName%>/jsp/media/map/static/js/chunk.js"></script>
	<script src="<%=webContextName%>/jsp/media/map/static/js/main.chunk.js"></script>
	
	<script type="text/javascript" src="<%= pathJS %>scanner.socket.js?v=2019-12-24"></script>
	...
	<div id="liabilityContentDiv">
	</div>
	<div id="sendMessageDlg" style="display:none;"> 
		<textarea id="sendMessageText" style="width: 500px; height: 250px;">Через одну минуту Вы будете отключены от сервера. Пожалуйста, сохраните измененные данные и выйдите из программы, для проведения работ на сервере!</textarea>
	</div>
</body>
</html>
<%
