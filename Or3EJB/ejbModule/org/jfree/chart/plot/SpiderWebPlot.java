...
public class SpiderWebPlot extends Plot implements Cloneable, Serializable {
    ...
    private Paint axisTickLabelPaint;
    ...
    public SpiderWebPlot(CategoryDataset dataset, TableOrder extract) {
        ...
        this.axisTickLabelFont = DEFAULT_LABEL_FONT;
        this.axisTickLabelPaint = DEFAULT_LABEL_PAINT;
    }
    ...
    public void setMaxValue(double value) {
    	if (value > 1000000000) {
    		...
    		gvWidth = Integer.parseInt(valueStr.substring(0, 1));
    		gvPrec = Integer.parseInt(valueStr.substring(1, 2));
    		this.gridValues = new double[(len - 2) / (gvWidth + gvPrec)];
    		for (int i = 0; i < this.gridValues.length; i++) {
    			int beg = 2 + i * (gvWidth + gvPrec);
    			int end = beg + gvWidth;
    			String gvStr = valueStr.substring(beg, end);
    			if (gvPrec > 0) {
    				gvStr += "." + valueStr.substring(end, end + gvPrec);
    			}
    			this.gridValues[i] = Double.parseDouble(gvStr);
    		}
    	} else {
    		this.gvPrec = 0;
    		this.gridValues = new double[5];
    		for (int i = 0; i < this.gridValues.length; i++) {
    			this.gridValues[i] = (i + 1) * value / 5;
    		}
    	}
		this.maxValue = this.gridValues[this.gridValues.length - 1];
        fireChangeEvent();
    }
    ...
    public void draw(Graphics2D g2, Rectangle2D area, Point2D anchor,
        ...
        if (!DatasetUtilities.isEmptyOrNull(this.dataset)) {
            ...
            if (this.gridValues != null) {
            	final String gvFmt = "%" + (gvWidth > 0 ? gvWidth : "") + "." + gvPrec + "f%%";
	            for (double value : this.gridValues) {
	                ...
	                for (int cat = 0; cat < catCount; cat++) {
	                    ...
	                    Point2D point = getWebPoint(radarArea, angle, value / this.maxValue);
	                ...
	                if (this.axisTickLabelFont != null) {
	                	final String valueStr = String.format(gvFmt, value);
	                	Rectangle2D bounds = this.axisTickLabelFont.getStringBounds(valueStr, g2.getFontRenderContext());
	                	g2.setFont(this.axisTickLabelFont);
	                	g2.setPaint(this.axisTickLabelPaint);
	                	g2.drawString(
	                			valueStr,
	                			(float)(labelPoint.getX() - bounds.getWidth() - 2),
	                			(float)(labelPoint.getY() + bounds.getHeight() - 2));
	                }
	            }
            }
            ...
            if (dataValue != null) {
                ...
                if (value >= 0) { // draw the polygon series...
                    ...
                    if (valueLabelFont != null) {
	                    final String valueStr = String.format("%,.1f%%", value);
	                    Rectangle2D valueBounds = this.valueLabelFont.getStringBounds(valueStr, g2.getFontRenderContext());
	                    g2.setFont(this.valueLabelFont);
	                    g2.setPaint(this.valueLabelPaint);
	                    g2.drawString(
	                    		valueStr,
	                    		(float)(point.getX() + (point.getX() < centre.getX() ? -(valueBounds.getWidth() + 5) : 5)),
	                    		(float)point.getY());
                    }
    ...
    protected void drawLabel(Graphics2D g2, Rectangle2D plotArea, double value,
        ...
        Rectangle2D labelBounds = getLabelFont().getStringBounds(lines[0], frc);
        final double lineHeight = labelBounds.getHeight();
        lineWidths[0] = labelBounds.getWidth();
        LineMetrics lm = getLabelFont().getLineMetrics(lines[0], frc);
        double ascent = lm.getAscent();
        
        for (int i = 1; i < lines.length; i++) {
            Rectangle2D lineBounds = getLabelFont().getStringBounds(lines[i], frc);
            labelBounds.setRect(
            		labelBounds.getX(),
            		labelBounds.getY(),
            		Math.max(labelBounds.getWidth(), lineBounds.getWidth()),
            		labelBounds.getHeight() + lineHeight);
            lineWidths[i] = lineBounds.getWidth();
        }
        ...
        for (int i = 0; i < lines.length; i++) {
	        g2.drawString(
	        		lines[i],
	        		(float) (labelLocation.getX() + (labelBounds.getWidth() - lineWidths[i]) / 2),
	                (float) (labelLocation.getY() + lineHeight * i));
        }
                                             ...
                                             double startAngle)
    {
        ...
        if (labelY > plotArea.getCenterY()) {
        	...
        	labelY -= labelBounds.getHeight() / 2;
        }
    ...
    public void setAxistTickLabelFont(final Font font) {
    	this.axisTickLabelFont = font;
    	fireChangeEvent();
    }

    public void setAxistTickLabelPaint(final Paint paint) {
    	this.axisTickLabelPaint = paint;
    	fireChangeEvent();
    }

    public void setValueLabelFont(final Font font) {
    	this.valueLabelFont = font;
    	fireChangeEvent();
    }

    public void setValueLabelPaint(final Paint paint) {
